...
if (missing.length > 0) throw new Error('PostgreSQL Production preflight contract requires: ' + missing.join(', '))
...
describe('OPT-6 Batch 3 Production preflight PostgreSQL execution contract', () => {
  ...
  afterAll(async () => {
    ...
    await client.query('DROP TABLE IF EXISTS public.sync_run CASCADE')
    await client.query('DROP TABLE IF EXISTS public.product CASCADE')
    await client.end()
  })

  it('executes the complete Production SELECT-only packet without syntax or CTE errors', async () => {
    const packet = readFileSync(resolve(process.cwd(), 'docs/reports/sql/2026-07-28-opt6-00052-production-preflight.sql'), 'utf8')
    ...
    expect(result.rows[0]).toHaveProperty('expected_version_name_digest_is_reviewed')
    expect(result.rows[0]).toHaveProperty('expected_history_payload_digest_is_reviewed')
