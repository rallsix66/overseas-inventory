...
type MatrixRow = {
  ...
  preferenceWrites: PreferenceWriteMatrix
}
...
type PreferenceWriteMatrix = {
  ownInsert: WriteAttempt
  crossInsert: WriteAttempt
  ownDelete: WriteAttempt
  crossDelete: WriteAttempt
}

async function attemptPreferenceWrite(
  ...
  try {
    ...
    const result = operation === 'insert'
      ? await client.query(
          'INSERT INTO public.user_variant_preference (id, user_id) VALUES ($1, $2)',
          [rowId, targetUserId],
        )
      : await client.query(
          'DELETE FROM public.user_variant_preference WHERE id = $1',
          [rowId],
        )
    return { ok: true, rowCount: result.rowCount ?? 0, errorCode: null, errorMessage: null }
  } catch (error) {
    ...
    return {
      ...
      errorMessage: typed.message ?? null,
    }
  } finally {
...
async function readPreferenceWrites(role: 'anon' | 'authenticated', userId?: string): Promise<PreferenceWriteMatrix> {
  const ownTarget = userId ?? ids.admin
  const crossTarget = userId === ids.admin ? ids.operator : ids.admin
  const ownRowId = userId === ids.admin ? 1 : userId === ids.operator ? 2 : userId === ids.otherOperator ? 3 : userId === ids.disabled ? 4 : 1
  const crossRowId = userId === ids.admin ? 2 : 1
  return {
    ownInsert: await attemptPreferenceWrite(role, userId, 'insert', ownTarget, 100),
    crossInsert: await attemptPreferenceWrite(role, userId, 'insert', crossTarget, 101),
    ownDelete: await attemptPreferenceWrite(role, userId, 'delete', ownTarget, ownRowId),
    crossDelete: await attemptPreferenceWrite(role, userId, 'delete', crossTarget, crossRowId),
  }
}

async function readMatrix(role: 'anon' | 'authenticated', userId?: string): Promise<MatrixRow> {
  await client.query('BEGIN')
  let row: Omit<MatrixRow, 'preferenceWrites'>
  try {
    await client.query(`SET LOCAL ROLE ${role}`)
    await client.query("SELECT set_config('request.jwt.claim.sub', $1, true)", [userId ?? ''])
    ...
    `, [userId ?? '00000000-0000-0000-0000-000000000000'])
    row = { ...result.rows[0], ownProfileUpdates: String(updated.rowCount ?? 0) }
  } finally {
    await client.query('ROLLBACK')
  }
  return { ...row, preferenceWrites: await readPreferenceWrites(role, userId) }
}
...
async function readTargetPolicyCatalog() {
...
describe('OPT-6 Migration 00050 PostgreSQL behavior contract', () => {
  ...
  it('preserves the complete identity and cross-user RLS matrix', async () => {
    ...
    expect(before).toEqual({
      anon: {
        ...
        preferenceWrites: expect.objectContaining({
          ownInsert: expect.objectContaining({ ok: false }),
          crossInsert: expect.objectContaining({ ok: false }),
          ownDelete: expect.objectContaining({ ok: false }),
          crossDelete: expect.objectContaining({ ok: false }),
        }),
      },
      admin: {
        ...
        preferenceWrites: expect.objectContaining({
          ownInsert: expect.objectContaining({ ok: true, rowCount: 1 }),
          crossInsert: expect.objectContaining({ ok: true, rowCount: 1 }),
          ownDelete: expect.objectContaining({ ok: true, rowCount: 1 }),
          crossDelete: expect.objectContaining({ ok: true, rowCount: 1 }),
        }),
      },
      operator: {
        ...
        preferenceWrites: expect.objectContaining({
          ownInsert: expect.objectContaining({ ok: true, rowCount: 1 }),
          crossInsert: expect.objectContaining({ ok: false, rowCount: null }),
          ownDelete: expect.objectContaining({ ok: true, rowCount: 1 }),
          crossDelete: expect.objectContaining({ ok: true, rowCount: 0 }),
        }),
      },
      otherOperator: {
        profiles: '4', ownProfile: '1', preferences: '1', ownPreferences: '1',
    ...
    const residue = await client.query('SELECT count(*)::text AS count FROM public.user_variant_preference')
    expect(residue.rows).toEqual([{ count: '4' }])
  })

  it.each([
    [
      'role drift',
      'ALTER POLICY "user_insert_own_preferences" ON public.user_variant_preference TO authenticated',
    ],
    [
      'complete WITH CHECK predicate drift',
      `ALTER POLICY "user_insert_own_preferences" ON public.user_variant_preference
       WITH CHECK (auth.uid() = user_id AND user_id <> '${ids.disabled}'::uuid)`,
    ],
    [
  ...
  ])('rejects %s before dropping any policy', async (_label, driftSql) => {
    await client.query(driftSql)
    const before = await readTargetPolicyCatalog()
    expect(before).toHaveLength(6)

    await expect(client.query(MIGRATION_SQL)).rejects.toThrow('OPT-6 baseline policy catalog drift')

    const after = await readTargetPolicyCatalog()
    expect(after).toEqual(before)
