...
describe('OPT-6 Batch 3 Production 00052 preflight static contract', () => {
  let packet = ''
  beforeAll(() => { packet = readFileSync(resolve(process.cwd(), 'docs/reports/sql/2026-07-28-opt6-00052-production-preflight.sql'), 'utf8') })
  ...
  it('pins the complete Production 00001-00051 history and payload', () => {
    ...
    expect(packet).toContain('expected_version_name_digest_is_reviewed')
    expect(packet).toContain('expected_history_payload_digest_is_reviewed')
    expect(packet).toContain("2d6174dce487614c3280456fff9169d0")
    expect(packet).toContain("0b7cba5a88fff139fb0ec65e4deaa142")
    expect(packet).toContain('00051_optimize_role_rls_policy_overlap')
    expect(packet).toContain("version = '00052'")
    expect(packet).toContain('Production SELECT-only preflight')
    expect(packet).not.toContain('Staging SELECT-only preflight')
    expect(packet).not.toContain('statements[1]')
  })

  it('pins the Production 00041-00047 payload variant', () => {
    expect(packet).toContain("('00041', '00041_replenishment_warehouse_params', 1, 752, 'dbe56c84bd30d389743043231452ec24')")
    expect(packet).toContain("('00047', '00047_dashboard_warehouse_health_overview', 1, 5519, '2e68149556947358eb11f41963a2b607')")
    expect(packet).not.toContain('adf5951cb448754b4a62e259a533eca1')
  })
