...
// 匹配/取消匹配操作仅 admin 可用（requireAdmin）
// 归档/恢复操作对所有已登录用户可用（requireActiveAuth），每个用户操作自己的偏好
...
/** 批量归档 Variant — 所有已登录用户均可操作（操作自己的偏好） */
export async function archiveVariants(
  ...
  try {
    ...
    const result = await variantRepository.archive(parsed.data.variantIds, user.id);
...
/** 批量恢复 Variant — 所有已登录用户均可操作（操作自己的偏好） */
export async function restoreVariants(
  ...
  try {
    ...
    const result = await variantRepository.restore(parsed.data.variantIds, user.id);
