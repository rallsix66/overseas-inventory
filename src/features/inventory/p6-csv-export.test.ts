...
describe('P6-CSV-EXPORT — 架构边界', () => {
  it('migrations/ 下无 00034 文件', () => {
    const m34 = path.resolve(process.cwd(), 'supabase/migrations/00034_');
    const files = fs.readdirSync(path.dirname(m34));
    const post33 = files.filter((f) => /^0003[4-9]|^000[4-9]|^00[1-9]/.test(f));
    expect(post33).toHaveLength(0);
  });
