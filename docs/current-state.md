...
**PERF-H-DROP-UNUSED-INVENTORY-QUANTITY-INDEXES**（2026-07-07 DONE）

## Current Task

**PERF-H-DROP-UNUSED-INVENTORY-QUANTITY-INDEXES** — 新增 Migration 00033 删除 `idx_inventory_low_stock` 和 `idx_inventory_quantity` 两个未使用索引（**DONE**，2026-07-07）。

### 背景

PERF-G 静态查询路径分析 + `pg_stat_user_indexes` 运行时统计确认两个索引均无使用：

- `idx_inventory_low_stock`：静态证据充分 — 所有低库存查询使用 `COALESCE(safety_stock, 0)` 动态参数，planner 无法证明 ≤500。运行时 `idx_scan=0 / idx_tup_read=0 / idx_tup_fetch=0 / index_size=16 kB`
- `idx_inventory_quantity`：`get_overseas_inventory` out_of_stock 分支虽有 `quantity = 0` 条件，运行时同样 `idx_scan=0 / idx_tup_read=0 / idx_tup_fetch=0 / index_size=16 kB`
...
**注意**：Migration 00033 已在 Supabase SQL Editor 手动执行成功（2026-07-07）。执行后 `pg_stat_user_indexes` 验证查询返回 No rows returned（两个索引已删除确认）。

### 限制

- 不修改 Migration 00001 中索引原始定义
- 不修改 RPC / RLS / 业务代码 / Repository
...
**Last Updated**: 2026-07-07 PERF-H Migration 00033 已在 Supabase SQL Editor 手动执行成功（DROP INDEX IF EXISTS idx_inventory_low_stock / idx_inventory_quantity，验证查询 No rows returned）。PERF-D-CACHE/PERF-D-OVERVIEW/PERF-F-TURBOPACK/PERF-G/PERF-H 已完成，Phase D/E Migration 已执行。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
