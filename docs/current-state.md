...
Phase C 性能优化（Phase B 基础已完成，Phase C 第一轮：Dashboard 首页查询并行重排已完成）

## Current Task

**PERF-C1** — Dashboard 首页查询并行重排（**DONE**，2026-07-04）。

PERF-C1 验收结论：
- 全量测试通过，build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有）
- **`src/app/dashboard/page.tsx`**：`getCurrentUser()` 之后，4 个独立查询（`getOverseasStats`、`getInTransitByVariant`、`getFollowedVariantsBasic`、`getLowStock`）通过 `Promise.all` 并行执行，消除串行 waterfall
- 每个查询保留独立 `.catch()` 错误处理：overseas stats → undefined（静默）、in-transit → empty Map（静默）、followed → followedError（展示错误）、low stock → lowStockError（展示错误）
- `inTransitQuantity` 注入和 `lowStockItems` map/sort 在并行块之后执行，保持字段语义
- 未登录时仅调用 `getOverseasStats`，不调用依赖 `user.id` 的三个查询
- **`src/features/inventory/p2-d2-low-stock-summary.test.ts`**：新增 10 项 PERF-C1 测试（Promise.all、独立 .catch()、注入/sort 顺序、无串行 await 残留）

| 子任务 | 内容 | 状态 |
|--------|------|------|
| page.tsx | Promise.all 并行执行 4 个独立查询，保留独立错误处理 | DONE |
| p2-d2-low-stock-summary.test.ts | 更新隔离测试 + 新增 10 项并行编排测试 | DONE |
| 文档 | current-state.md / current-task.md 收口 | DONE |

下一步：PERF-C1 完成。Phase C 其余内容（产品页 actions 并行、海外库存 actions 并行、同步页分页、索引优化）未开始。P3-S1B 仍 BLOCKED_EXTERNAL。可推进 PERF-C2（产品页/海外库存页 actions 并行）或其他未阻塞任务。
...
- **PERF-C2 待推进**：产品页/海外库存页 actions 并行重排（其他页面 waterfall 优化）。
- **Phase D/E 待推进**：同步页分页、索引优化。
