...
P5-SY11A — Migration 00011：`is_archived` 列 + 审计字段 + 部分索引 + RLS 调整（Operator SELECT 过滤 `is_archived = false`）。P5-SY11B~F 后续子任务待 Migration 完成后依次推进。P5-SY10 Phase B 自动 Real Write 设计预留，当前不启用。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。
...
- `src/features/variants/repository.ts` — 新增 `archive()` / `restore()` 方法；`list()` / `getUnmatched()` 增加 `archiveStatus` 过滤；`match()` / `unmatch()` / `batchMatch()` 阻止已归档操作
- `src/features/variants/actions.ts` — 新增 `archiveVariants()` / `restoreVariants()` Server Actions（Admin only）
- `src/features/variants/types.ts` — `VariantFilters` 增加 `archiveStatus`
- `src/features/variants/schema.ts` — 新增 archive/restore Zod schemas
- `src/features/inventory/repository.ts` — `getOverseasList()` / `getLowStock()` / `getOverseasStats()` 过滤已归档 Variant
- `src/app/dashboard/variants/page.tsx` — 替换占位符：数据表格 + 归档筛选标签 + Admin 归档/恢复按钮
- `src/app/dashboard/variants/unmatched/page.tsx` — 替换占位符：仅活跃未匹配列表
- `src/types/database.ts` — product_variant 类型增加 `is_archived` / `archived_at` / `archived_by`
- `src/features/variants/components/` — `variant-table.tsx` / `archive-controls.tsx`
