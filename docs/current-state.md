...
- **Phase 3 内部路径（S2A~S6）全部 DONE**。下一步可选：P3-S5B（Admin 部分入仓 / Admin 批量入仓，按需拆分）、P3-S1B 恢复（百世 API 授权后）、Phase 4（用户管理）、Phase 6（国内库存）。当前业务口径：Admin 维护在途和入仓，Operator 只读查看已分配仓库数据。
