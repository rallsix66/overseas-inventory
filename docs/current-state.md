...
**P6-CSV-EXPORT**（2026-07-07 DONE）

## Current Task

**P6-CSV-EXPORT** — 海外库存 CSV 导出（**DONE**，2026-07-07）。

### 功能

- 海外库存页面新增"导出 CSV"按钮，复用当前筛选条件
- Server Action `exportOverseasInventoryCsv` 使用分页循环（pageSize=100）拉取，最多 10000 行
- 在途数据由 `getInTransitConfirmedAggregate` 聚合后按 (variantId, warehouseId) 维度回填
- CSV 带 UTF-8 BOM，逗号分隔，RFC 4180 双引号转义
- Blob 下载，文件名 `overseas-inventory-YYYYMMDD.csv`

### 核心文件

| # | 文件 | 说明 |
|---|------|------|
| 1 | `src/lib/csv.ts`（新增） | `toCsv(rows, columns)` 纯函数 |
| 2 | `src/features/inventory/actions.ts`（修改） | 新增 `exportOverseasInventoryCsv` + 10 列定义 |
| 3 | `src/features/inventory/schema.ts`（修改） | 新增 `exportCsvSchema` |
| 4 | `src/app/dashboard/inventory/overseas/_components/overseas-page-content.tsx`（修改） | 导出按钮 + handleExportCsv |
...
**Last Updated**: 2026-07-07 P6-CSV-EXPORT DONE（海外库存 CSV 导出：pageSize=100 分页循环 + getInTransitConfirmedAggregate 在途回填 + UTF-8 BOM + 36 项测试，不新增 Migration/RPC/RLS，不改 Repository 签名）。PERF-H Migration 00033 已执行。PERF-D-CACHE/PERF-D-OVERVIEW/PERF-F-TURBOPACK/PERF-G/PERF-H 已完成。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
