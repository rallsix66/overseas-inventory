...
`P3-S2E` — 在途入口收口 + 采购单号 + 海外库存轻量展开（**DONE**，2026-06-30）
...
**Current Task**: P3-S2E DONE（2026-06-30）。P3-S2D DONE。P3-S1B CODE COMPLETE / BLOCKED_EXTERNAL。P3-S3 DONE。详情见下方 Next Step。
...
| RLS | 46 条策略，全部启用 |
| 新 Migration（P3-S2E） | 00020（purchase_order_no + create_shipment_transactional 11 参数 admin-only）已就绪；00021（change_shipment_status_transactional admin-only 覆盖）已就绪 |
...
| 日期 | 变更 |
|---|---|
| 2026-06-30 | **P3-S2E DONE。在途入口收口 + 采购单号 + 海外库存轻量展开。** (1) Migration 00020：`shipment.purchase_order_no` 字段 + `create_shipment_transactional` RPC 11 参数 Admin-only + `RETURNING id`。(2) Migration 00021：`change_shipment_status_transactional` 覆盖为 Admin-only。(3) 采购单号全链路：types → Zod → Repository → Actions → 创建/编辑表单 + 列表副行 + 详情页。(4) `shipmentRepository.getInTransitDetailsByVariantAndWarehouse()` — 按 (variantId, warehouseId) 查询在途明细（单号/采购单号/在途数量/预计到货），`InTransitDetailRow` 客户端组件集成海外库存行展开。(5) 入口收口：侧边栏移除"在途库存"，`/dashboard/inventory/in-transit` → redirect；Operator 隐藏写按钮。(6) 权限链：Server Action + RPC 双层 Admin-only。(7) 44 项 P3-S2E 测试（17 repository 行为 + 27 action/Zod/源码）。全量 1603/1604（49 文件，2 预存失败），lint 0 errors / 26 warnings，build pass。 |
