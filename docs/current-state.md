...
**PERF-F-TURBOPACK 移除 --webpack 切换默认 Turbopack**（2026-07-07 DONE）

## Current Task

**PERF-F-TURBOPACK** — 移除 `--webpack` flag，切换 Next.js 16 默认 Turbopack（**DONE**，2026-07-07）。

`package.json` 原有 `next dev --webpack` / `next build --webpack`，显式降级到 webpack。Next.js 16 默认使用 Turbopack（Rust 增量打包器），无需额外 flag。

**实现**：
- 移除 `package.json` dev/build 脚本中的 `--webpack` flag
- 修复 Turbopack 构建发现的 1 个真实 Bug：`warehouse-access/actions.ts` 中 `export type { ... }` type-only re-export 在 `'use server'` 模块中不产生运行时值，webpack 静默容忍，Turbopack 正确拒绝。移除该行（无人从 actions.ts 导入这两个类型，全部直接从 `./types.ts` 导入）
- **不添加 `optimizePackageImports`**：项目唯一使用的 `lucide-react` 已在 Next.js 默认自动优化列表中；无其他重型多导出库需要手动配置
- `perf-f-turbopack.test.ts`：9 项静态测试（package.json 不含 `--webpack` + next.config.ts 存在）
- 已确认构建使用 Turbopack：`▲ Next.js 16.2.9 (Turbopack)`
...
**Last Updated**: 2026-07-07 PERF-F-TURBOPACK DONE（移除 --webpack，Next.js 16 默认 Turbopack 构建通过；修复 1 个 type-only re-export Bug；不添加 optimizePackageImports）。PERF-D-CACHE/PERF-D-OVERVIEW/PERF-F-TURBOPACK 完成，Phase D/E Migration 已执行。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。后续如重新生成 Supabase 类型，可替换手动添加的 database.ts RPC 类型声明。
