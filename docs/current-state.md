...
Phase B 性能优化（Phase B 第一轮已完成：request-scope cache + dashboard layout 接入）

## Current Task

**PERF-B1** — DIS 性能优化 Phase B 第一轮：request-scope cache + dashboard layout 接入（**DONE**，2026-07-04，收口修复完成）。

PERF-B1 验收结论：
- 全量 2754/2754（65 文件）+ 1 项 PGRST116 新增测试，build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有）
- **`src/lib/auth.ts`**：新增 `cachedGetAuthProfile` React `cache()` → 合并 `auth.getUser()` + `profiles(display_name, is_active, role)` 一次查询；`getCurrentUser()` 和 `getCurrentActiveUser()` 复用同一缓存
- **`src/app/dashboard/layout.tsx`**：移除直接 `createClient().auth.getUser()` + profiles 查询，改用 `getCurrentUser()` 共享 helper
- **`src/features/shipments/repository.ts`**：`getUserRole` 包裹 `cache()`，语义不变
- **`src/features/warehouse-access/repository.ts`**：新增 `cachedGetAccessibleWarehouseIds`（返回 `string[]`），公开方法返回 `new Set()`
- **`src/features/variants/repository.ts`**：新增 `cachedGetUserArchivedVariantIds`（返回 `string[]`），公开方法返回 `new Set()`
- **PGRST116 行为测试**：`getCurrentActiveUser()` 在 profile 查询返回 PGRST116 时返回 `null`（与未登录/停用一致），行为已测试确认

| 子任务 | 内容 | 状态 |
|--------|------|------|
| auth.ts | cachedGetAuthProfile cache() + getCurrentUser/getCurrentActiveUser 复用 | DONE |
| dashboard/layout.tsx | 替换直接 Supabase 调用为 getCurrentUser() | DONE |
| shipments/repository.ts | getUserRole 包裹 cache() | DONE |
| warehouse-access/repository.ts | cachedGetAccessibleWarehouseIds 返回 string[] | DONE |
| variants/repository.ts | cachedGetUserArchivedVariantIds 返回 string[] | DONE |
| 测试 | auth.test.ts select 适配 + PGRST116 行为测试 | DONE |
| 文档 | current-state.md / current-task.md 收口 | DONE |

下一步：PERF-B1 完成。LOW-STOCK-PAGINATION、UNMATCHED-PAGINATION、PERF-S1 均已完成。P3-S1B 仍 BLOCKED_EXTERNAL。可推进 Phase C（页面查询并行重排）或其他未阻塞任务。
...
- **Phase C 待推进**：页面查询并行重排（dashboard pages + Server Actions 已有 request-scope cache，并行重排可减少 waterfall）。
- **Phase D/E 待推进**：同步页分页、索引优化（与 PERF-B1 无冲突，可独立推进）。
...
**Last Updated**: 2026-07-04 LOW-STOCK-PAGINATION DONE（RPC 返修完成，runtime smoke test 通过）。getLowStock() Migration 00028 RPC 补齐，已手动执行并验证。PERF-S1 全系列、P4-UX、P2-D2、SIDEBAR-ENABLE、UNMATCHED-PAGINATION、LOW-STOCK-PAGINATION 均已完成。全量 2754/2754（65 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL，可推进其他未阻塞任务。
