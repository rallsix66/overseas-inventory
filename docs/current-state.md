...
**NEXTJS16-PROXY-MIGRATION**（2026-07-08）

## Current Task

**NEXTJS16-PROXY-MIGRATION** — `middleware.ts` 迁移至 `proxy.ts`（Next.js 16 推荐）。

### 功能

- `src/middleware.ts` 迁移为 `src/proxy.ts`，消除 Next.js 16 middleware deprecation warning
- 导出函数 renamed: `middleware` → `proxy`，matcher 配置不变
- `updateSession` cookie/session 刷新逻辑完全不变
- `/dashboard/*` 未登录 → `/auth/login` 重定向；`/auth/login` 已登录 → `/dashboard` 重定向
...
| 3 | `src/lib/supabase/middleware.ts`（修改） | 注释更新 |
| 4 | `src/lib/supabase/server.ts`（修改） | 注释更新 |
| 5 | `src/proxy.test.ts`（新增） | 21 项静态/结构测试 |

### 限制

- 不修改登录页面 UI
- 不修改认证业务语义
- 不修改数据库、Migration、RPC、RLS
- 不修改同步真实写入逻辑
...
**Current Task**: NEXTJS16-PROXY-MIGRATION — `src/middleware.ts` → `src/proxy.ts` 迁移 + 注释更新 + 21 项测试 + 文档同步。P3-S5B 全部完成：B0 ✅ B1 ✅ B2 ✅ B3 ✅ B4 ✅ B5 ✅。P3-S1B（百世 API 恢复）待后续任务（BLOCKED_EXTERNAL）。
