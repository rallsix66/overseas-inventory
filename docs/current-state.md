...
**Phase E 索引优化**（2026-07-07 DONE + REVISED + EXECUTED）

## Current Task

**PHASE-E** — 索引优化（**DONE + REVISED + EXECUTED**，2026-07-07 返工修正并执行 00031）。

Phase E 基于 2026-07-07 真实查询路径审查（sync_run 分页/并发锁、shipment 在途/入仓、shipment_item 聚合、overseas inventory 反连接、tracking_event 时间线排序），新增 Migration 00031 补建 **7 个索引**。

**返工修正（2026-07-07）**：
- 删除 `idx_inventory_variant_id`：00001 `inventory_variant_warehouse_unique UNIQUE (variant_id, warehouse_id)` 自动生成的唯一索引 `variant_id` 前导列已覆盖单列查询
- 删除 `idx_sync_run_warehouse_status`：00007 `idx_sync_run_one_in_progress ON sync_run(warehouse_id) WHERE status='in_progress'` 部分唯一索引精确覆盖 claim_sync_run 热路径
- `idx_shipment_status_created` → `idx_shipment_active_created`：改为部分索引 `ON shipment(created_at DESC) WHERE status <> 'warehoused'`，更精确覆盖 `list()` 主查询的 `.neq('status','warehoused').order('created_at',{ascending:false})`

**Migration 00031** — `00031_phase_e_index_optimization.sql`（仅索引，未修改表结构/RPC/RLS/权限）：
1. `idx_sync_run_warehouse_started` — `(warehouse_id, started_at DESC)`：消除 sync 分页/历史查询排序
2. `idx_sync_run_status_lease` — `(status, lease_expires_at)`：cleanup_expired 租约扫描（lease_expires_at 此前无索引）
3. `idx_shipment_warehouse_status` — `(warehouse_id, status)`：在途明细/聚合过滤
4. `idx_shipment_active_created` — `(created_at DESC) WHERE status <> 'warehoused'`：在途列表主查询/批量入仓排序（部分索引，created_at 此前无索引）
5. `idx_shipment_item_shipment_variant` — `(shipment_id, variant_id)`：在途明细双列过滤
6. `idx_uvp_variant_user_type` — `(variant_id, user_id, preference_type)`：海外库存/低库存 LEFT JOIN 反连接
7. `idx_tracking_event_shipment_occurred` — `(shipment_id, occurred_at)`：详情页轨迹时间线排序

**phase-e-indexes.test.ts**：35 项静态契约测试（仅索引变更 / 7 索引 + 命名 / 目标表列 / schema 不破坏 / 幂等 / 元数据）。
...
**Last Updated**: 2026-07-07 Phase E EXECUTED（索引优化 — Migration 00031 返工修正后已在 Supabase SQL Editor 手动执行成功：删除 idx_inventory_variant_id/idx_sync_run_warehouse_status 两个冗余索引，idx_shipment_status_created 改为部分索引 idx_shipment_active_created；7 个索引，35 项静态契约测试，全量 2905/2905 通过，build pass，lint 5e/25w 均为既有）。Phase D/E 均已完成并执行生产 Migration。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
