...
`P3-S1B` — 百世 API Client、签名与 Dry Run 拉取（**REWORK**，2026-06-28）
...
- `P3-S1B` — 百世 API Client、签名与 Dry Run 拉取（2026-06-28 **REWORK**。三轮复验 1 项数据完整性阻塞已修复。`src/lib/providers/best/` 模块：types.ts / signature.ts / schema.ts / parse-response.ts / client.ts / dry-run.ts / index.ts + 3 测试文件（88 项测试）。核心修复：① BestApiResponse 新增 rawData: unknown — API envelope 原始 data，不受 Zod 剥离影响，供 raw_payload 写入；② post() 同时返回 data（raw）和 rawData；③ query 方法 data=Zod 校验后的类型化通用字段，rawData=未经变换的原始数据；④ dryRunWaybill/dryRunOrder 的 orderInfo/logisticsTrace 使用 rawData（保留所有 provider 专有字段），itemSummary/trackingSummary 使用 Zod data 生成（仅通用字段）。新增 5 项未知字段保留 + 深度相等测试。1350/1350 测试通过，lint 0 errors / 24 warnings，build 通过。协议 SPECULATIVE，真实 Dry Run 待用户提供官方 API 文档和凭证。不进入 P3-S1C。
...
**Current Task**: P3-S1B REWORK（2026-06-28，三轮复验修复完成）。Migration 00017 已由用户确认在 Supabase SQL Editor 执行。`src/lib/providers/best/` 模块：types.ts / signature.ts / schema.ts（Zod 校验）/ parse-response.ts / client.ts / dry-run.ts / index.ts + 3 测试文件（88 项测试）。API 协议标记为 SPECULATIVE。三轮修复：BestApiResponse 新增 rawData: unknown 保留原始 provider data（Zod 不剥离未知字段）；query 方法同时返回 data（Zod 校验后）和 rawData（原始）；dryRun orderInfo/logisticsTrace 使用 rawData，itemSummary/trackingSummary 使用校验后 data。新增未知字段保留/深度相等测试。原始 provider data 完整保留，Zod 解析不覆盖 raw payload。1350/1350 测试通过，lint 0 errors / 24 warnings，build 通过。真实 Dry Run 待用户提供百世官方 API 文档、BEST_OPEN_* 凭证和测试单号。在真实协议确认和真实只读 Dry Run 成功前不得标 DONE，不得进入 P3-S1C。
...
| In-Transit Inventory 页面 | Phase 3 | P3-S1A DONE：Migration 00017 已由用户手动执行；P3-S1B REWORK |
| Shipments 页面 | Phase 3 | P3-S1A DONE：数据模型已就绪；P3-S1B REWORK（API 协议 SPECULATIVE，真实 Dry Run 待执行） |
...
- Domestic Inventory、Users 页面仍为占位实现；In-Transit Inventory / Shipments 已进入 Phase 3 开发（P3-S1A DONE / P3-S1B 待开始）
...
1. **百世 API 协议未确认**：端点 URL、Content-Type、请求字段名、成功码均基于通用文档推断，尚未经真实 API 调试验证。需用户提供百世开放平台官方接口文档或真实凭证后核对。
2. **缺少 BEST_OPEN_* 环境变量**：`.env.local` 无 BEST_OPEN_BASE_URL / BEST_OPEN_PARTNER_ID / BEST_OPEN_SECRET，无法执行真实只读 Dry Run。
3. **P3-S1B 独立验收未通过**：状态为 REWORK，修复后再验收。在真实协议确认和真实 Dry Run 成功前不得标 DONE，不得进入 P3-S1C。

## Next Step

用户提供百世开放平台官方接口文档（queryOrderInfoByOrderNo / 物流轨迹接口的真实 URL、请求方式、Content-Type、参数名、成功码和响应结构；bizData 序列化及 MD5 大小写规则）和 BEST_OPEN_* 凭证后，核对并更新 SPECULATIVE 标记，执行真实只读 Dry Run，通过独立验收后标 DONE。
...
2026-06-28 — P3-S1B REWORK（三轮复验修复完成）。1 项修复：BestApiResponse 新增 rawData: unknown 字段保留 API 原始 data（Zod z.object() 默认剥离未知字段），query 方法同时返回 data（Zod 校验后）和 rawData（未经变换），dryRun orderInfo/logisticsTrace 使用 rawData 保留 provider 专有字段供 raw_payload 写入，itemSummary/trackingSummary 使用校验后 data。新增 5 项未知字段保留 + 深度相等测试。88 项 best/ 测试 + 1350/1350 全量测试通过，lint 0 errors / 24 warnings，build 通过。API 协议 SPECULATIVE，真实 Dry Run 待百世官方文档和凭证确认。完成后停止等待复验。真实协议确认和真实只读 Dry Run 成功前不得标 DONE，不得进入 P3-S1C。
