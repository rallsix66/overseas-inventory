...
`P5-SY10` — 自动 Dry Run 预审与后续自动化分阶段框架（IN_PROGRESS — P5-SY10A DONE，P5-SY10B DONE。规则引擎 + 历史上下文提供器已实现。`getWarehouseHistory()` 从 sync_run 推导 hasBaseline / consecutiveFailures / lastSuccess / stats。617/617 TS 测试，lint 0 errors，build pass。下一步 P5-SY10C。）
...
- `P5-SY10B` — 历史上下文提供器：基线追踪 + 连续失败检测（2026-06-24 DONE。SyncRepository 接口新增 `getWarehouseHistory(warehouseId)`，MockRepository + SupabaseSyncRepository 实现。从 sync_run 推导 hasBaseline / consecutiveFailures / lastSuccess / stats（最近 5 次完成均值）。`sync-service.ts` 中 `buildResultSummary()` 将 scraperMeta 字段（rawRowCount/validSkuCount/invalidSkuCount）持久化到 result_summary JSONB。31 项测试覆盖冷启动/有基线/连续失败 1~3/成功重置/跨仓隔离/缺失 result_summary/混合格式。质量门：617/617 TS 测试，lint 0 errors，build pass。不新增 DB 表，不改 Migration。）

## Awaiting Review

- P5-SY10B~F 子任务待启动。P5-SY11 ProductVariant 软归档与库存视图降噪记入后置计划。
...
P5-SY10A DONE — 规则引擎核心已实现并验收通过。下一步 P5-SY10B（历史上下文提供器：基线追踪 + 连续失败检测），从 sync_run/sync_log 推导 hasBaseline / consecutiveFailures / stats，走 Repository 接口。P5-SY11 软归档后置，当前不启动。
...
2026-06-24（P5-SY10A DONE。规则引擎 `evaluateRules()` 纯函数已实现，11 条规则 R1~R11，冷启动/有基线双路径。586/586 TS 测试，lint 0 errors，build pass。P5-SY9 全部子任务 A~K DONE。WEBSYNC_REAL_WRITE_ENABLED=false。下一步 P5-SY10B。）
