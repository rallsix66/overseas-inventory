...
**P6-OVERSEAS-PRODUCT-NAME-VISIBILITY**（2026-07-08）
...
- 海外库存表格"产品名称"列移到 SKU 前面，作为主要识别列
- productName 为空时显示"未匹配产品"文字 + 黄色 Badge"未匹配"标记
- 产品名称列 max-w-[180px] truncate 防止撑破表格
- 搜索 placeholder 保持"搜索 SKU 或产品名称..."
- Repository `mapOverseasRow` 确认 product_name → productName 映射正确

### 核心文件

| # | 文件 | 说明 |
|---|------|------|
| 1 | `src/app/dashboard/inventory/overseas/_components/overseas-page-content.tsx`（修改） | 列顺序调整 + productName 空值 Badge |
...
**Current Task**: P6-OVERSEAS-PRODUCT-NAME-VISIBILITY — 海外库存表格列顺序调整 + productName 空值 Badge + 34 项测试 + 文档同步。P3-S5B 全部完成。P3-S1B（百世 API 恢复）待后续任务（BLOCKED_EXTERNAL）。
