...
**PERF-D-CACHE 缓存收口**（2026-07-07 DONE）

## Current Task

**PERF-D-CACHE** — 移除同步模块海外仓进程级缓存（**DONE**，2026-07-07）。

`src/features/sync/server-actions.ts` 原有模块级 `let _overseasWhCache` + `getCachedOverseasWarehouses()` 进程级缓存。该缓存在仓库增删改名后会 stale，且与 Phase B 已建立的 request-scope cache 模式不一致。

**修复**：
- 删除 `_overseasWhCache` 变量和 `getCachedOverseasWarehouses()` 函数
- 所有 7 处调用点改为直接 `await getOverseasWarehouses()`（每次查询 `warehouse` 表，~5 行，开销可忽略）
- 新增 `perf-d-cache.test.ts`：8 项静态源码测试，证明无模块级缓存残留

**Phase E 索引优化**（已完成，不重复展开）：
- Migration 00031 返工修正后 7 个索引，已在 Supabase SQL Editor 手动执行成功
...
- **PERF-D-CACHE**（DONE，2026-07-07）：移除 server-actions.ts 模块级 `_overseasWhCache` 进程级缓存。8 项静态源码测试，全量 2913/2913 通过（70 文件），build pass，lint 仅既有 5e/25w。
...
**Last Updated**: 2026-07-07 PERF-D-CACHE DONE（移除 server-actions.ts 模块级 `_overseasWhCache` 进程级缓存，改为每次直接查询；8 项静态源码测试 + 注释引用修正，全量 2913/2913 通过 70 文件，build pass，lint 5e/25w 均为既有）。Phase D/E 均已完成并执行生产 Migration。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
