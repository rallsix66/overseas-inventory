...
`UNMATCHED-PAGINATION` — 待处理 SKU 页面分页补齐 + Pending Modules 文档修正（**DONE**，2026-07-04）。

UNMATCHED-PAGINATION 验收结论：
- 全量 2717/2717（65 文件），build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有），git diff --check pass
- `variantRepository.getUnmatched()` 改为对象参数 `{ userId, page, pageSize }`，返回 `PaginatedResult<VariantItem>`，DB 层 `count: 'exact'` + `.range()` + `notIn` 归档过滤在分页前
- `unmatched/page.tsx` 从 searchParams 读取 page + 分页导航（上一页/下一页/第 N 页，共 M 条），PAGE_SIZE = 20
- Pending Modules 表修正：Users 已完成、Overseas Inventory 已稳定、In-Transit 内部路径已完成
- Technical Debt 更新：`getUnmatched()` 分页已补齐，`getLowStock()` 仍保留

| 子任务 | 内容 | 状态 |
|--------|------|------|
| repository 修改 | getUnmatched 签名改为对象参数 + PaginatedResult + count exact + range + notIn | DONE |
| page.tsx 修改 | searchParams 读取 page + 分页导航 Link | DONE |
| 测试 | p5-sy11g-e-ui.test.ts 新增 10 项断言（参数签名/PaginatedResult/count exact/range/notIn 位置/searchParams/分页导航） | DONE |
| 文档 | current-state.md / current-task.md / Pending Modules 修正 / Technical Debt 更新 | DONE |

下一步：待处理 SKU 分页补齐完成。P3-S1B 仍 BLOCKED_EXTERNAL。可推进其他未阻塞任务（getLowStock 分页补齐 或 P2-I4 Domestic Inventory）。
...
- 海外库存查询已由 PERF-S1（Migration 00027）迁移至数据库 RPC 函数（get_overseas_inventory / get_overseas_stats / get_low_stock_rpc），不再使用 JS 全量过滤分页
...
- `getLowStock()` 无分页，数据量大时需补（UNMATCHED-PAGINATION 已为 `getUnmatched()` 补齐分页）
...
- **UNMATCHED-PAGINATION**（DONE，2026-07-04）：待处理 SKU 页面分页补齐 + Pending Modules 文档修正。getUnmatched 改为分页查询（count exact + range + notIn）+ unmatched/page.tsx 分页导航 + 10 项新测试。全量 2717/2717（65 文件），lint 仅既有 5e/25w，build pass。不新增 Migration/RPC。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-04 UNMATCHED-PAGINATION DONE。待处理 SKU 分页补齐 + Pending Modules 修正。PERF-S1 全系列、P4-UX、P2-D2、SIDEBAR-ENABLE、UNMATCHED-PAGINATION 均已完成。全量 2717/2717（65 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL，可推进其他未阻塞任务。
