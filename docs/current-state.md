...
**P6-OVERSEAS-INVENTORY-UI-CLARITY**（2026-07-08 DONE）

## Current Task

**P6-OVERSEAS-INVENTORY-UI-CLARITY** — 海外库存 UI 清晰度优化（多阶段）。

### 阶段

本任务分多个阶段推进，每个阶段独立验收：

- **阶段 0**：需求固化，写入文档，不改 src 代码 ✅
- **阶段 A**：筛选器中文化 + 统计卡片可点击切换 + 防页面跳顶
- **阶段 B**："已确认到仓"列从海外库存主表移除（在途管理/批量入仓内部保留）
- **阶段 C**：展开物流明细增加"最近物流更新时间"
- **阶段 D**：未匹配海外库存行预留"绑定产品"入口（仅 UI 占位，不实现真实绑定）
- 各阶段可并行或顺序推进，按用户确认激活
...
- **产品列表**（`/dashboard/products`）= DIS 标准产品主数据
- **海外库存页**继续显示 BigSeller 原始品名（当前 productName 字段）
- **未匹配海外 SKU** 行旁预留"绑定产品"按钮入口 → 弹出可搜索的标准产品列表 → 确认后创建 ProductVariant（在 Product → ProductVariant → Inventory 模型内完成绑定）
- 长期总看板方向：`标准产品名 → 国内库存 → 各海外市场库存 → 在途库存 → 国内生产周期 → 各市场运输周期`
- 禁止绕过 Product → ProductVariant → Inventory 模型
- **真实产品绑定功能另开后续任务，不在本任务实现**

#### 2. "已确认到仓"字段处理

- **海外库存主表移除**该列及相关字样
- 原因：BigSeller 抓取后的 `inventory.quantity` 已经是"当前库存"（含已入库库存），主表再并列展示"已确认到仓"容易让运营误以为库存之外还有额外数量
- **在途管理**（`/dashboard/shipments`）和**批量入仓**（`/dashboard/shipments/batch`）内部流程保留该口径
- 不修改对应 RPC（`get_in_transit_confirmed_aggregate`）、Migration、RLS

#### 3. 物流更新时间

- 主表"同步成功 + 时间"仍表示 BigSeller 库存同步时间（`last_sync_at`）
- 展开行底部新增"最近物流更新时间"，显示最近一条 tracking_event 或 shipment 更新时间
- 数据字段选择：
  - 优先用 `tracking_event.occurred_at`（最近一个 tracking 事件时间，最精确反映物流实际动态）
  - 如无 tracking_event，fallback 到 `shipment.updated_at`（最近一次状态推进或编辑）
  - 如该 (variantId, warehouseId) 下无任何在途 shipment，显示"—"
- 数据获取方式：
  - 最小变更方案：复用现有 `getInTransitDetailsByVariantAndWarehouse` repository 方法返回的数据
  - 在该方法查询结果中添加 `latestTrackingAt: string | null` 字段（从 tracking_event 聚合或 shipment.updated_at 计算）
...
- 筛选器下拉显示中文而非裸 `all`：
  ...
  - "全部仓库"（不是 "all"）
  - "全部状态"（不是 "all"）
- 统计卡片可点击：低库存、在途库存等卡片点击后在当前页面展示对应 SKU 信息（通过设置筛选条件实现，如点击"低库存"→ stockStatus='low'）
- 换页或筛选时**防止页面跳回顶部**（使用 `router.push` 的 `scroll: false` 选项或 `window.scrollTo` 控制）

#### 5. 分页（后续独立优化项）

- BigSeller 风格分页（如页码按钮 + 省略号 + 跳转输入框）作为后续独立 UI 优化项
- 当前分页 UI（上一页/下一页 + "共 N 条，第 X/Y 页"）暂时保留
- 不在本任务第一阶段实现，视实现范围控制决定是否纳入后续阶段

### 第一阶段实现范围

以上 1~4 中标记为 UI 可立即实现的项：

| 项目 | 说明 | 阶段 |
|------|------|------|
| 筛选器中文化 | "全部国家/全部仓库/全部状态" | A |
| 统计卡片可点击 | 低库存/在途卡片点击设置筛选 | A |
| 防页面跳顶 | `router.push` 加 `scroll: false` | A |
| 移除"已确认到仓"列 | 主表移除，内部保留 | B |
| 展开行物流更新时间 | 新增 tracking_event.occurred_at 展示 | C |
| 预留"绑定产品"入口 | 未匹配行 UI 占位按钮 | D |
...
**全阶段 A~D 已实现并验收。** 质量门：3090/3091 测试（1 预存失败）、build pass、lint 0/26。
...
**Current Task**: P6-OVERSEAS-INVENTORY-UI-CLARITY 全阶段 A~D DONE。P3-S5B 全部完成。P3-S1B（百世 API 恢复）待后续任务（BLOCKED_EXTERNAL）。
...
- **下一步**：继续完成剩余 2 天手动真实同步观察；满 5 天后再评估 Cron/自动同步。
...
**Last Updated**: 2026-07-08 P6-OVERSEAS-INVENTORY-UI-CLARITY DONE（全阶段 A~D：筛选器中文化 + 统计卡片可点击 + scroll:false 防跳顶 + 已确认到仓列移除 + 展开行物流更新时间 + 绑定产品 UI 占位）。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
