...
`PERF-S1D` — 关键按钮局部更新（**DONE**，2026-07-03）。P3-S5B 全部完成（B0~B5 ✅）。海外库存列表/统计/在途+已确认聚合已接入 00027 三个 RPC（`get_overseas_inventory` / `get_overseas_stats` / `get_in_transit_confirmed_aggregate`），移除 JS 全量过滤分页和 N+1 查询。PERF-S1C 的聚合内容（合并在途 + 已确认到仓聚合、消除 `getConfirmedWarehousedByWarehouse` 按仓循环查询）已并入 PERF-S1B 完成。PERF-S1D 已移除关注按钮、确认到仓、BigSeller 吸收等操作的 `router.refresh()`，改为局部状态更新或回调式局部刷新。下一步：**PERF-S1E**（质量门/文档收口/性能验收）。
...
**Current Task**: P3-S5B5（应用行为测试 + 文档同步 + 质量门）— **DONE**（2026-07-03）。P3-S5B4 DONE + 返修完成（Zod 校验 + colSpan 修复）。P3-S5B3 DONE（详情页双模式按钮 + PartialWarehouseDialog + BigsellerAbsorptionButton，2026-07-02 收口返修完成）。P3-S5B2 DONE（Repository + Actions + 87 项测试，已返修聚合口径）。P3-S5B1 DONE（Migration 00026 + types/schema + 93 项静态测试）。P3-S5B0 DONE（旧 00023 应用层入口已封存）。P3-S6 DONE（2026-06-30）。P3-S5A DONE（Migration 00023 已执行并验证）。P3-S4A DONE（Migration 00022 已执行）。在途模块内部路径（S2A~S5A）全部完成，权限与 RLS 验收通过。P3-S5B 全部完成：B0 ✅ B1 ✅ B2 ✅ B3 ✅ B4 ✅ B5 ✅。P3-S1B（百世 API 恢复）待后续任务。Migration 00026 已于 2026-07-02 手动执行并验证通过。
...
- **PERF-S1A**（DONE，2026-07-03 两次返修）：Migration 00027（get_overseas_inventory / get_overseas_stats / get_in_transit_confirmed_aggregate 三个 RPC）+ 79 项静态契约测试。已执行并通过数据库侧 smoke 验证（2026-07-03：三个 RPC 存在、SECURITY INVOKER、anon 拒绝、auth.uid() 绑定正确）。
- **PERF-S1B**（DONE，2026-07-03）：Repository（getOverseasList/getOverseasStats/getInTransitConfirmedAggregate）+ Actions（getOverseasInventory）接入 00027 三个 RPC。移除 JS 全量过滤分页和 N+1 confirmedMap 查询。PERF-S1C 的聚合内容（合并在途+已确认到仓聚合、消除按仓循环查询）已并入本任务完成。
- **PERF-S1D**（DONE，2026-07-03）：关键按钮局部更新，减少 router.refresh()。关注按钮：移除 router.refresh()，仅乐观更新。确认到仓：dialog 新增 onSuccess 回调 → PartialWarehouseEntry → ShipmentDetailClient 局部获取最新数据。BigSeller 吸收：本地 absorbed 状态隐藏按钮 + onSuccess 回调。新增 ShipmentDetailClient 管理在途详情页交互区本地状态。批量入仓页已使用 loadPage() 局部刷新无需修改。全量 2639/2639（63 文件），build pass，lint 5 errors / 25 warnings（all pre-existing），git diff --check pass。下一步：**PERF-S1E**（质量门/文档收口/性能验收）。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-03 PERF-S1D DONE。关键按钮局部更新，减少 router.refresh()：关注按钮（仅乐观更新）、确认到仓/ BigSeller 吸收（onSuccess → ShipmentDetailClient 局部刷新）。全量 2639/2639（63 文件），lint 5 errors / 25 warnings（all pre-existing），build pass。下一步 PERF-S1E。
...
2026-07-03 — PERF-S1D DONE。关键按钮局部更新（关注/确认到仓/BigSeller 吸收去 router.refresh()），新增 ShipmentDetailClient 管理在途详情页交互区本地状态。全量 2639/2639（63 文件），lint 5 errors / 25 warnings（all pre-existing），build pass。下一步 PERF-S1E。
