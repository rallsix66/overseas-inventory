...
`P5-SY12D` — Dashboard 关注产品动态运营可用性收口（IN PROGRESS，2026-06-26）
...
**Current Task**: P5-SY12D DONE（2026-06-26）。等待用户确认下一任务。
...
无阻塞。P5-SY12D 已完成，等待用户确认下一任务。

## Next Step

P5-SY12D 已完成（2026-06-26）：Dashboard 关注产品动态运营可用性收口 — 新增状态筛选（全部/紧急/低库存/正常/数据不足）、每行跳转入口（海外库存 SKU 搜索）、未匹配 SKU 明确提示（不参与 safety_stock 判断，仍可用 estimated_days 动态告警）、边界状态（筛选无结果/加载失败/空关注/全未匹配）。不改数据模型/Migration。质量门：1014/1014 TS 测试，lint 0 errors / 24 warnings（pre-existing），build pass。等待用户确认下一任务，不自动进入相邻任务。P5-SY10 Phase B 自动 Real Write 设计预留，当前不启用。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。
...
2026-06-26 — P5-SY12D DONE。Dashboard 关注产品动态运营可用性收口完成：新增 FollowedProductsSection Client Component（状态筛选 全部/紧急/低库存/正常/数据不足 + 每行跳转海外库存 + 未匹配 tooltip 提示 + 筛选无结果/加载失败/空关注/全未匹配边界状态）。Dashboard page.tsx 数据获取链路不变（Repository Pattern）。不改数据模型/Migration。Migration 00013（preference_type `'favorited'` 扩展）已于 2026-06-25 手动执行。Migration 00014（inventory.daily_sales/estimated_days + warehouse.lead_time_days + RPC 扩展）已于 2026-06-26 手动执行并生产验证。质量门：1014/1014 TS 测试（35 文件），lint 0 errors / 24 pre-existing warnings，build pass，Python compileall pass。`product_variant.is_archived` 为遗留列。等待用户确认下一任务。
