...
`P5-SY12C-RUNTIME` — Dashboard 关注产品动态告警升级（DONE + production runtime verified，2026-06-26）
...
**Current Task**: P5-SY12C-RUNTIME DONE（2026-06-26）。等待用户确认下一任务。
...
无阻塞。P5-SY12C-RUNTIME 已完成生产验证，等待用户确认下一任务。

## Next Step

P5-SY12C-RUNTIME 已完成生产验证（2026-06-26）。Migration 00014 已在 Supabase SQL Editor 手动执行成功；5 个海外仓 lead_time_days=30；daily_sales 全部写入，estimated_days 按 BigSeller 可用数据写入（部分 SKU 为空属正常）；Dashboard 关注区动态告警正常运作。Migration 00013（preference_type 'favorited' 扩展）已于 2026-06-25 手动执行。质量门：1006/1006 TS 测试，lint 0 errors，build pass。等待用户确认下一任务，不自动进入相邻任务。P5-SY10 Phase B 自动 Real Write 设计预留，当前不启用。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。
...
2026-06-26 — P5-SY12C-RUNTIME DONE + production verified。阶段 C 动态告警升级完成：Migration 00014（inventory.daily_sales/estimated_days + warehouse.lead_time_days + RPC sync_warehouse_inventory 扩展）+ Python sync 链路透传校验 + FollowedVariantBasic 扩展 + 动态告警规则（estimated_days < lead_time_days → critical, quantity < safety_stock → warning）+ Dashboard 日销/可售天数/补货周期 + alertLevel badge。Migration 00014 已手动执行成功；5 仓 lead_time_days=30；daily_sales 全部写入，estimated_days 按 BigSeller 可用数据写入（部分 SKU 为空属正常）；PH 仓 expired in_progress 已清理。Migration 00013（preference_type `'favorited'` 扩展）已于 2026-06-25 手动执行。质量门：1006/1006 TS 测试（35 文件），lint 0 errors / 24 pre-existing warnings，build pass，Python compileall pass。`product_variant.is_archived` 为遗留列。等待用户确认下一任务。
