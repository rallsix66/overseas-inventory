...
`P4-UX` — 用户管理页局部刷新收口（**DONE**，2026-07-03）。

P4-UX 验收结论：
- 全量 2660/2660（63 文件），build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有），git diff --check pass
- `UserDetailSheet`：移除 `useRouter` / `router.refresh()`，操作成功后通过 `getUserById(userId)` 局部刷新 Sheet 详情 + `onUserChanged?.()` 通知父组件，Sheet 保持打开不关闭
- `UsersPageContent`：新增 `users` / `localTotal` 本地 state + `useEffect` 同步 `data`/`initialTotal` props（筛选/分页导航后覆盖旧数据）+ `handleUserChanged` callback 通过 `listUsers` 局部刷新列表
- `rg "router.refresh|useRouter" src/features/users`: 仅测试断言，无业务代码使用
- `rg "router.refresh|useRouter" src/app/dashboard/users`: 仅 `users-page-content.tsx` 保留 `useRouter` 用于筛选/分页 `router.push` 导航

| 子任务 | 内容 | 状态 |
|--------|------|------|
| P4-UX 代码 | UserDetailSheet onUserChanged + getUserById 局部刷新；UsersPageContent useEffect props 同步 + listUsers 局部刷新 | DONE |
| P4-UX 测试 | p4-u2.test.ts 新增 8 项 P4-UX 断言（useEffect + props 同步 + 无 router.refresh + onUserChanged + 导入控制 + handleUserChanged + useRouter 仅导航） | DONE |
| P4-UX 文档 | current-state.md / current-task.md 同步 | DONE |

下一步：PERF-S1 全系列 + P4-UX 均已完成。Phase 3 主线 P3-S1B 等待百世 API 授权恢复，或推进其他未阻塞任务。
...
- **PERF-S1E**（DONE，2026-07-03）：质量门 / 文档收口 / 性能验收。全量 2652/2652，build pass，lint 仅既有 smoke-test-00025.ts 5e/25w。PERF-S1 全系列完成。
- **P4-UX**（DONE，2026-07-03）：用户管理页局部刷新收口。移除 `UserDetailSheet` 内 `useRouter`/`router.refresh()`，P4-U3/P4-U4 成功后改为 `getUserById` 局部刷新 Sheet + `onUserChanged` 通知父组件 + `listUsers` 局部刷新列表。全量 2660/2660（63 文件），build pass，lint 仅既有 smoke-test-00025.ts 5e/25w。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-03 P4-UX DONE。PERF-S1 全系列（A/B/C/D/E）+ P4-UX 均已完成。全量 2660/2660（63 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。下一步回到 Phase 3 主线。
