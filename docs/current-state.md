...
**P6-OVERSEAS-INVENTORY-UX-V2**（2026-07-08 A+B+C+D DONE + D 返工 DONE + 搜索语义收口 DONE + 列宽防重叠 DONE — BigSeller 分页 + 统计卡片真实联动 + 筛选状态可见化 + 真实产品绑定 + 字段语义修正 + 分词搜索 + 写后校验 + 搜索语义收口审查 + 列宽防重叠修复）

## Current Task

**P6-OVERSEAS-INVENTORY-UX-V2** — 海外库存完整体验优化（A+B+C+D 已实现，E 仅计划）。

> **注意**：上一轮 P6-OVERSEAS-INVENTORY-UI-CLARITY（全阶段 A~D DONE）仅完成轻量 UI 清晰度修正。本轮 V2 补齐分页、筛选可见化、卡片联动、真实产品绑定等运营刚需体验。

### V2 功能范围

| 功能 | 说明 | 状态 |
|------|------|------|
| A. BigSeller 风格分页 | 页码按钮 + 省略号 + 每页条数 20/50/100 | ✅ DONE |
| B. 统计卡片真实联动 | 卡片点击后筛选状态可见；在途卡片保持不可点击（需后端扩展） | ✅ DONE |
| C. 筛选状态可见化 | 筛选标签/摘要行 + 单清 + 全清 + URL 同步 | ✅ DONE |
| D. 产品绑定真实功能 | Dialog 搜索标准产品 → Server Action 绑定 → ProductVariant 更新 → router.refresh | ✅ DONE |
| E. 产品看板长期方向 | 仅记录不实现：标准产品 → 国内外库存 → 在途 → 周期 | PLAN ONLY |

A+B+C+D 质量门：3208/3209 测试（1 预存 WEBSYNC_REAL_WRITE_ENABLED）、build pass、lint 0 errors / 25 warnings（all pre-existing）。
...
- `P6-OVERSEAS-INVENTORY-UX-V2 D 返工` — 字段语义修正 + 搜索增强 + 写后校验（2026-07-08 **DONE**。① **字段语义修正**：新增 `InventoryItem.variantName`（BigSeller 原始品名 = product_variant.name）、`standardProductName`（DIS 标准产品名 = product.name）、`standardProductCode`（DIS 标准产品编码 = product.code）。`productName` 保持向后兼容但语义改为 BigSeller 品名。海外库存列表主品名列显示 BigSeller 品名，标准产品名仅作为辅助信息展示。matched + standardProductName 为空 → "已匹配标准品缺失"。Migration 00034 新增 `get_overseas_inventory` / `get_low_stock` RPC 返回 `v.name AS variant_name`。② **搜索增强**：`searchProducts` 改用 `productRepository.search` 分词搜索（按空格/连字符/下划线/斜杠/括号拆 token → 每个 token ILIKE code + name → 同时通过 product_variant.sku 反向查找 Product → 仅返回启用产品 → escapeLike 防止破坏 .or() 语法）。③ **写后校验**：`bindOverseasVariant` 成功后读回 variant.getById 校验 product_id / match_status / 关联 product 可读，失败返回中文错误不假成功。④ 新增 60+ 项测试。3269/3270 测试、build pass、lint 0 errors / 25 warnings（all pre-existing）。）
- `P6-OVERSEAS-INVENTORY-UX-V2 搜索性能 + 列宽拖拽修复 + 语义收口`（2026-07-08 **DONE**。① **搜索性能**：Migration 00036 启用 pg_trgm 扩展 + 为 product_variant.sku/name 和 product.name/code 建 GIN trigram 索引，加速 ILIKE '%keyword%' 模糊搜索。不修改 00034/00035，不改变搜索逻辑/函数签名/RLS/权限模型。00035 解决搜索准确性（连续子串 + 分词 AND），00036 通过 trigram index 优化性能。未来数据量继续增大时再考虑 dedicated search_vector / materialized search_text。**注意**：00036 的 product_variant.name 索引不用于绑定产品搜索（绑定搜索仅使用 code/name/sku，product_variant.name 为 BigSeller 原始品名非标准变体名称）。name 索引仍服务于 RPC 00035 的库存列表搜索（p_search 需匹配 BigSeller 品名）。② **列宽拖拽修复**：表格改为 tableLayout fixed + totalTableWidth 控制；resize handle 从透明 w-1.5 改为可见分隔线（w-6 命中区 + w-px 竖线，默认灰 hover/drag 蓝，title/aria-label）；封装 ResizeHandle 组件消除重复；新增 activeResizeKey 高亮拖拽中列；未匹配分支保持 flex w-full min-w-0 + badge/button shrink-0。③ **列宽防重叠修复**：仓库列 overflow-hidden truncate + COL_MIN.warehouse 110→140 + COL_MIN.productName 260→280 + localStorage v2 key + 列结构 12/12/12 一致性确认。④ **绑定搜索语义收口审查**：product_variant.name 从绑定搜索字段中移除（仅保留 code/name/sku）；variantName 仅作为 query seed 预填搜索框关键词；UI 文案修正为「搜索标准产品名称、编码或 SKU」「未找到匹配的标准产品」；如后续需 BigSeller 品名提升召回，应独立实现 alias / keyword mapping / search_document。）
...
**Current Task**: P6-OVERSEAS-INVENTORY-UX-V2 A+B+C+D DONE（BigSeller 分页 + 统计卡片真实联动 + 筛选状态可见化 + 真实产品绑定）。E（产品看板长期方向）仅计划。P3-S1B（百世 API 恢复）待后续任务（BLOCKED_EXTERNAL）。

### 同步验证状态

- **手动真实同步验证**：已连续 3/5 天正常（2026-07-08 更新）。
- **WEBSYNC_REAL_WRITE_ENABLED=true**：仅表示允许人工触发真实写入，不代表自动同步开启。
- **自动同步启用前置条件**：需连续 5 天手动同步成功后，才进入自动同步评估。当前进度 3/5 天，自动同步仍未启用。
...
| 新 Migration（P6-UX-V2-D） | 00034（get_overseas_inventory / get_low_stock RPC 增加 variant_name 字段 — BigSeller 原始品名 via product_variant.name）已创建，待手动执行 |
...
- **下一步**：P6-OVERSEAS-INVENTORY-UX-V2 A+C 已完成。B（统计卡片真实联动）/D（产品绑定真实功能）待用户确认。继续完成剩余 2 天手动真实同步观察；满 5 天后再评估 Cron/自动同步。
...
**Last Updated**: 2026-07-08 P6-OVERSEAS-INVENTORY-UX-V2 A+C DONE（BigSeller 风格分页：页码按钮 + 省略号 + 每页 20/50/100；筛选状态可见化：国家/仓库/状态/搜索标签 + X 清除 + 清空筛选）。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
