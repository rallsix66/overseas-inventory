...
**PERF-G-IDX-INVENTORY-LOW-STOCK-EVALUATION 评估 idx_inventory_low_stock**（2026-07-07 DONE）

## Current Task

**PERF-G-IDX-INVENTORY-LOW-STOCK-EVALUATION** — 评估 `idx_inventory_low_stock` 是否可删除（**DONE**，2026-07-07）。

### 评估结论

**`idx_inventory_low_stock`：建议删除**（静态证据充分：所有低库存查询均使用动态参数，planner 无法证明 ≤500）。
**`idx_inventory_quantity`：候选删除，删除前需运行时验证**（`get_overseas_inventory` out_of_stock 分支仍有 `p_stock_status = 'out_of_stock' AND i.quantity = 0`，静态分析不能完全排除命中可能）。

### 证据链

**索引定义**（`00001_initial_schema.sql:128-129`）：
...
两个均为部分索引（partial index），只覆盖子集行。

**查询路径全覆盖分析**：

| # | 查询路径 | 方式 | quantity 条件 | 命中 partial index? |
|---|---------|------|--------------|---------------------|
| 1 | `getOverseasList()` → `get_overseas_inventory` RPC | RPC | `quantity <= COALESCE(p.safety_stock, 0)`（动态参数） | ❌ planner 无法证明 ≤500 |
| 2 | `getLowStock()` → `get_low_stock` RPC | RPC | `quantity <= COALESCE(p.safety_stock, 0)`（动态参数） | ❌ 同上 |
| 3 | `getOverseasStats()` → `get_overseas_stats` RPC | RPC | `quantity <= COALESCE(safety_stock, 0)`（动态参数） | ❌ 同上 |
| 4 | `list()` | PostgREST | 无 quantity 过滤，仅 `order('quantity', { ascending: true })` | ❌ 无 WHERE 子句；排序跨全部行而 partial index 只含 ≤500 |
| 5 | `getByProductId()` | PostgREST | 无 quantity 过滤，仅 `.eq('variant.product_id', id)` | ❌ |
| 6 | `getFollowedVariantsBasic()` (preferences) | PostgREST | 无 quantity 过滤，`.in('variant_id', [...])` | ❌ |
| 7 | `canAccessVariant()` (warehouse-access) | PostgREST | 无 quantity 过滤，`.eq('variant_id', ...)` | ❌ |
| 8 | `getById()` (products) | PostgREST | 无 quantity 过滤，`.eq('variant.product_id', id)` | ❌ |
| 9 | `updateQuantity()` | PostgREST | `.eq('id', inventoryId)` | ❌ |
| 10 | `sync_warehouse_inventory` RPC (00006/00009/00014) | RPC | JOIN on variant_id + warehouse_id | ❌ 写操作 |
| 11 | `warehouse_shipment_transactional` RPC (00023) | RPC | ON CONFLICT (variant_id, warehouse_id) | ❌ 写操作 |
| 12 | RLS policies (46 条) | RLS | user_id / role / warehouse_id 过滤 | ❌ 无 quantity 条件 |

**关键原因**：所有低库存/海外库存查询已由 PERF-S1（Migration 00027/00028）迁移至 RPC，使用 `COALESCE(p.safety_stock, 0)` 动态参数。PostgreSQL 查询计划器无法在计划时证明动态参数 `≤ 500`，因此不会选择 partial index。

`idx_inventory_quantity WHERE quantity = 0` 的潜在使用者是 `get_overseas_inventory` RPC 中的 `p_stock_status = 'out_of_stock' AND i.quantity = 0` 分支。虽然也是参数化条件，但 `out_of_stock` 是海外库存页三个主要筛选状态之一，业务流量较高，静态分析不能完全排除 planner 在特定参数下选择该 partial index 的可能。因此标记为候选删除，需运行时验证。

### 限制与下一步

- 纯静态评估，未连接 Supabase 数据库运行 `EXPLAIN` 或查询 `pg_stat_user_indexes`
- **下一步**：在 Supabase SQL Editor 执行以下查询，结合实际业务流量判断：
  ```sql
  SELECT
    schemaname,
...
- PERF-D-CACHE：移除模块级进程缓存
- PERF-D-OVERVIEW：仓库概览服务端化（Migration 00032 已执行）
- PERF-F-TURBOPACK：移除 --webpack，Turbopack 构建通过
- Phase D/E：分页 + 索引优化（Migration 00029/00030/00031 已执行）
...
**Last Updated**: 2026-07-07 PERF-G 返修口径（idx_inventory_low_stock 建议删除 / idx_inventory_quantity 候选删除需运行时验证；idx_scan=0 不代表从未使用，需结合业务流量+EXPLAIN 判断）。下一步：建议在 Supabase SQL Editor 执行 pg_stat_user_indexes 查询。PERF-D-CACHE/PERF-D-OVERVIEW/PERF-F-TURBOPACK 已完成，Phase D/E Migration 已执行。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
