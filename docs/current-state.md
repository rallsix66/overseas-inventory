...
Phase C 性能优化（Phase B 基础已完成，PERF-C1 Dashboard 首页 + PERF-C2A 海外库存 actions 已完成）

## Current Task

**PERF-C2A** — 海外库存 getOverseasInventory 查询编排优化（**DONE**，2026-07-04）。

PERF-C2A 验收结论：
- 全量测试通过，build pass，lint 仍为既有 5 errors / 25 warnings（`smoke-test-00025.ts`），未发现本轮新增 lint error
- **`src/features/inventory/actions.ts`**：重构 `getOverseasInventory` 查询编排 — `getOverseasWarehouses` / `getOverseasList` 与 `getInTransitConfirmedAggregate` 提前并行启动（不再等待 aggregate 完成）；`getOverseasStats` 等 `variantTotalMap` 构建完毕后加入第二轮 `Promise.all`。新增 `seal()` helper 防止提前启动的 promise 产生 unhandledRejection。
- `whInTransitMap` / `variantTotalMap` / `confirmedMap` 构建逻辑不变；`inTransitQuantity` 注入语义不变
- 错误处理不变：任一查询失败仍抛出错误，由 `error.tsx` 边界处理
- **`src/features/shipments/p3-s5b4-batch-warehouse.test.ts`**：新增 10 项 PERF-C2A 编排测试

| 子任务 | 内容 | 状态 |
|--------|------|------|
| actions.ts | aggregate/warehouses/list 提前并行 + stats 第二轮并行 | DONE |
| p3-s5b4-batch-warehouse.test.ts | 新增 10 项 PERF-C2A 查询编排测试 | DONE |
| 文档 | current-state.md / current-task.md 收口 | DONE |

下一步：PERF-C2A 完成。可推进 PERF-C2B（产品页 actions 并行重排）或其他未阻塞任务。P3-S1B 仍 BLOCKED_EXTERNAL。
...
**Last Updated**: 2026-07-04 PERF-C2A DONE（海外库存 getOverseasInventory 查询编排优化，aggregate/warehouses/list 提前并行）。PERF-S1 全系列、PERF-B1、PERF-C1、P4-UX、P2-D2、SIDEBAR-ENABLE、UNMATCHED-PAGINATION、LOW-STOCK-PAGINATION 均已完成。全量测试通过，lint 仍为既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL。PERF-C2B（产品页）待推进。
