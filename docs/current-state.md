...
Phase 3 — 在途与物流（P3-S5B 部分/批量入仓）

## Current Task

`P3-S5B4` — 批量入仓 UI + 海外库存"已确认到仓"列（**DONE**，2026-07-02）
...
- `P3-S5B4` — 批量入仓 UI + 海外库存"已确认到仓"列（2026-07-02 **DONE**）。**批量入仓 UI**：新建 `/dashboard/shipments/batch` 路由（Server Component Admin-only + `BatchWarehousePage` 客户端组件）。列表展示 customs + 已分配仓库的 eligible shipments（复用 `listEligibleForBatchWarehousing`），支持选择/全选/展开查看产品明细（`getShipmentDetail` 按需加载）。每行展开后显示产品明细表格，支持数量输入 + 字段级校验（`validateEntry`：小数/负数/零/超量中文错误）+ "全额确认"一键填入在途余量。批量提交调用 `batchWarehouseShipments` action（逐笔串行，单笔失败不影响后续），提交结果显示每笔成功/失败，已成功 shipment 自动从选中列表移除并刷新。空数据/加载/错误/提交中状态完整覆盖。分页 20 条/页。新增 `listEligibleForBatchWarehousingAction` Server Action（Admin-only + Zod + 仓库隔离）。**海外库存"已确认到仓"列**：`getOverseasInventory` action 新增每仓并行查询 `getConfirmedWarehousedByWarehouse`（单仓失败不阻塞页面），返回 `confirmedMap`（warehouseId→variantId→confirmedQuantity）。`OverseasPageContent` 表格新增"已确认到仓"列，从 `confirmedMap` 读取对应行的已确认数量，仅展示 customs 或 warehoused + bigseller_absorbed_at IS NULL 的 DIS 确认事实。colSpan 更新为 14。**侧边栏**：物流组下新增"批量入仓"链接（`PackageCheck` 图标，Admin-only）。新增 61 项源码级测试（海外库存数据层 12 + 批量页 Server Component 5 + Client Component 33 + listEligible Action 7 + 侧边栏 5 + 架构合规 4 ，扣减 2 重复）。权限审计测试适配（actions 计数 12→13）。**不写 inventory.quantity**，不调 00023 RPC，不新增 Migration。质量门：shipments 881/881（14 文件），全量 2443/2443（61 文件），build pass，lint 5 errors / 26 warnings（all pre-existing）。
...
**Current Task**: P3-S5B5（应用行为测试 + 文档同步 + 质量门，待开始）。P3-S5B4 DONE。P3-S5B3 DONE（详情页双模式按钮 + PartialWarehouseDialog + BigsellerAbsorptionButton，2026-07-02 收口返修完成）。P3-S5B2 DONE（Repository + Actions + 87 项测试，已返修聚合口径）。P3-S5B1 DONE（Migration 00026 + types/schema + 93 项静态测试）。P3-S5B0 DONE（旧 00023 应用层入口已封存）。P3-S6 DONE（2026-06-30）。P3-S5A DONE（Migration 00023 已执行并验证）。P3-S4A DONE（Migration 00022 已执行）。在途模块内部路径（S2A~S5A）全部完成，权限与 RLS 验收通过。P3-S5B 拆分进度：B0 ✅ B1 ✅ B2 ✅ B3 ✅ B4 ✅ B5 待后续。P3-S1B（百世 API 恢复）待后续任务。Migration 00026 已于 2026-07-02 手动执行并验证通过。
...
| 函数 | `get_user_role` / `handle_new_user` / `update_updated_at_column` / `create_shipment_transactional` / `batch_match_variants` / `sync_warehouse_inventory` / `claim_sync_run` / `claim_sync_run_system` / `release_sync_run` / `heartbeat_sync_run` / `cleanup_expired_sync_runs` / `get_sync_runs` / `get_sync_run_detail` / `trg_sync_warehouse_lock_insert` / `change_shipment_status_transactional` / `warehouse_shipment_transactional` / `update_user_role_protected` / `toggle_user_active_protected` / `check_operator_profile_update` |
...
| In-Transit Inventory 页面 | Phase 3 | P3-S1A DONE（Migration 00017 已执行）；P3-S1B CODE COMPLETE / BLOCKED_EXTERNAL（百世 API 权限未开通）；P3-S3 DONE（手动补录） |
| Shipments 页面 | Phase 3 | P3-S1A DONE；P3-S1B CODE COMPLETE / BLOCKED_EXTERNAL；P3-S2A/S2B DONE；P3-S3 DONE（手动创建/补录）；P3-S2E DONE（入口收口 + 采购单号 + 海外库存展开） |
...
- Domestic Inventory、Users 页面仍为占位实现；In-Transit Inventory / Shipments 已进入 Phase 3 开发（P3-S1A DONE / P3-S1B CODE COMPLETE / BLOCKED_EXTERNAL / P3-S3 DONE / P3-S2A DONE）
...
| 日期 | 变更 |
|---|---|
| 2026-07-02 | **P3-S5B4 DONE。批量入仓 UI + 海外库存"已确认到仓"列。** (1) 新建 `/dashboard/shipments/batch` 路由：Server Component Admin-only + `BatchWarehousePage` 客户端组件（checkbox 全选/选择 + 行展开加载产品明细 + 逐项数量校验 + "全额确认"一键填入 + 批量提交结果汇总）。(2) `listEligibleForBatchWarehousingAction` Server Action（Admin-only + 仓库隔离）。(3) `getOverseasInventory` 新增每仓并行查询 `getConfirmedWarehousedByWarehouse`，返回 `confirmedMap`（warehouseId→variantId→confirmedQuantity）。(4) 海外库存页新增"已确认到仓"列（口径仅 customs 或 warehoused + bigseller_absorbed_at IS NULL）。(5) 侧边栏物流组下新增"批量入仓"链接（PackageCheck 图标，Admin-only）。(6) 新增 61 项源码级测试；权限审计测试适配（actions 12→13）。全量 2443/2443（61 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。不写 inventory.quantity，不调 00023 RPC，不新增 Migration。 |
...
- **P4-U1**：DONE — 返工完成（2026-07-01）。初始修复 7 项 + 返工修复 5 项真实行为风险。63 项测试（+13 返工），2018/2018 tests，lint 0 errors / 24 warnings，build pass。
- **Phase 3 内部路径（S2A~S6）全部 DONE**。Phase 4 用户管理已开始。
- **P4-U2**：DONE（2026-07-01）。用户列表只读页面完成。Admin-only 访问，searchParams 筛选（状态+角色）+ 分页 + 行详情 Sheet。42 项测试。2060/2060 tests。
- **P4-U5**：DONE（2026-07-01）。安全审计：TOCTOU 竞态发现并修复（Migration 00024 RPC 原子保护）。25 项 Migration 静态契约测试。2150/2150 tests。Phase 4 全模块完成。
- **P3-S5B**（Admin 部分入仓 / Admin 批量入仓，按需拆分）。
- **P3-S1B**：CODE COMPLETE / BLOCKED_EXTERNAL。恢复条件：百世确认 partnerId 已授权 `queryOrderInfoByOrderNo` / `trackingQuery` 两个只读接口。
- **P3-S1C**：依赖 P3-S1B，BLOCKED。
- **P3-S1D**：依赖 P3-S1C，BLOCKED。
- **P3-S2**：依赖 P3-S1D，BLOCKED（完整在途列表与详情含百世双源展示）。
- **P3-S4**：依赖 P3-S2 + P3-S3，BLOCKED（百世路径状态推进）。

**Last Updated**: 2026-07-02 P3-S5B3 DONE + 收口返修完成。详情页双模式按钮 + PartialWarehouseDialog (Zod 校验，无 parseInt 静默截断) + BigsellerAbsorptionButton。PartialWarehouseEntry 渲染条件收口（Admin + customs + warehouse_id）。全量 2381/2381 测试（60 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。
...
2026-07-02 — P3-S5B3 DONE + 收口返修完成。详情页双模式按钮 + PartialWarehouseDialog (Zod 校验，无 parseInt 静默截断) + BigsellerAbsorptionButton。PartialWarehouseEntry 渲染条件收口（Admin + customs + warehouse_id）。全量 2381/2381 测试（60 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。下一步 P3-S5B4。
