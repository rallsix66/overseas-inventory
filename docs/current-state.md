...
**P7-PRODUCT-OVERVIEW**（2026-07-10 P7-PLAN DONE，Codex 复验通过。标准产品总看板前置方案已确认，P7-MVP 待启动。P6-OVERSEAS-INVENTORY-UX-V2 已完整闭合。）

## Current Task

**P7-PLAN** — DONE（Codex 复验通过）。P7「标准产品总看板」前置方案已完成，下一步进入 P7-MVP 实现。当前阶段不实现代码。

> **背景**：P6 E 阶段将"产品看板长期方向"记录为 `标准产品名 → 国内库存 → 各海外市场库存 → 在途库存 → 国内生产周期 → 各市场运输周期`。P7 目标是在不新增数据库模型、不接入新数据源的前提下，基于现有真实数据（Product + ProductVariant + Inventory oversea + Shipment/ShipmentItem）构建标准产品总看板。
...
| P7-MVP | 标准产品总看板只读页面：Product 维度汇总 5 市场库存+在途+告警，国内库存占位 | PLAN |
...
| 决策项 | 结论 | 理由 |
|--------|------|------|
| 新 RPC | 暂不新增（待验证） | 以 `product` 表为分页主表 + 已有 RPC 作为变体级数据补充 + 应用层聚合。禁止以 `get_overseas_inventory` 作为 Product 维度分页驱动。若实现阶段发现 Product 维度筛选/排序/权限隔离复杂度过高，重新评估新增只读 RPC `get_product_overview` |
| 新 Migration | 不新增 | 全部从已有表读取 |
| 新 RLS | 不新增 | 不建新表 |
| 国内库存 | 仅占位「待接入」 | 不展示假数据/假表/假字段/假 RPC |
| 页面路由 | `/dashboard/products/overview` | 产品分组下新增侧边栏入口 |
...
- `P7-PRODUCT-LIST-EXPAND` — 产品列表展开行与编辑 Sheet 体验增强（2026-07-10 **DONE**，Codex 复验通过。含一轮视觉返工。① 产品列表表格新增展开/收起按钮，展开后显示 SKU 绑定明细（国内 SKU + 海外仓 SKU 统一表含国家/SKU/仓库产品名/匹配状态/最后同步列），复用现有匹配状态 badge 和时间格式。② 编辑 Sheet 产品编码不再 disabled；后端 updateProduct + repository.update 支持更新 code，含重复编码校验（应用层预检 + DB 23505 兜底）。③ 编辑 Sheet 内新增 SKU 绑定展示区（只读，add 模式不展示）。④ ProductItem 新增 bindings?: ProductSkuBindingSummary；productRepository.list() 批量查询 product_variant 代替仅做计数。⑤ 视觉返工：编辑 Sheet 宽度修复为 760px（`!max-w-[760px]` 覆盖默认 `sm:max-w-sm`）；Sheet 改为 Header / Body（独立滚动）/ Footer 三段式布局；SKU 绑定区从"每国家一张小表"改为统一表格（国家 badge + 等宽 SKU + 仓库产品名 truncate + 匹配状态/最后同步 whitespace-nowrap + overflow-x-auto）；列表展开行同步改为统一 SKU 表格 + 统计摘要行。不改 Migration/RPC/RLS，保持 Repository Pattern + Server Actions + RLS。文件范围：types.ts / repository.ts / actions.ts / product-form.tsx / products-page-content.tsx / product-detail-client.tsx / product-list-expand-bindings.test.ts。质量门：product-list-expand-bindings.test.ts 59/59 + 产品模块 79/79 + 全量 3409/3415（6 个 P6 海外库存旧断言失败非本任务引入）、lint 0 errors / 25 warnings、build pass、git diff --check pass。）
...
**Current Task**: P7-PLAN DONE，Codex 复验通过（2026-07-10）。下一步 P7-MVP。P6-OVERSEAS-INVENTORY-UX-V2 已关闭（2026-07-09 FINAL CLOSED）。P3-S1B（百世 API 恢复）待后续任务（BLOCKED_EXTERNAL）。
...
- **下一步**：P7-PLAN DONE，Codex 复验通过。进入 P7-MVP 实现（标准产品总看板只读页面）。自动同步仍保持未启用，连续手动真实同步观察不属于当前 P7 阻塞项。
...
**Last Updated**: 2026-07-10 — **P7-PRODUCT-OVERVIEW / P7-PLAN DONE**（Codex 复验通过）。P6-OVERSEAS-INVENTORY-UX-V2 FINAL CLOSED（A+B+C+D+F 全部完成，E 仅计划，P6-RECOVERABLE-FIX + P6-UX-IN-TRANSIT-OPTIMIZATION DONE）。P7-MVP 待启动。自动同步仍未启用，连续手动真实同步观察不属于当前 P7 阻塞项。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。
