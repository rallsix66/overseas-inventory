...
`PERF-S1B` — Repository / Server Action 接入 RPC（**DONE**，2026-07-03，Migration 00027 未执行生产）。P3-S5B 全部完成（B0~B5 ✅）。海外库存列表/统计/在途+已确认聚合已接入 00027 三个 RPC，移除 JS 全量过滤分页和 N+1 查询。下一步：PERF-S1C（合并在途+已确认到仓聚合，消除 confirmedMap N+1）。
...
- **PERF-S1A**（DONE，2026-07-03 两次返修）：Migration 00027（get_overseas_inventory / get_overseas_stats / get_in_transit_confirmed_aggregate 三个 RPC）+ 79 项静态契约测试。未执行生产 Migration。
- **PERF-S1B**（DONE，2026-07-03）：Repository（getOverseasList/getOverseasStats/getInTransitConfirmedAggregate）+ Actions（getOverseasInventory）接入 00027 三个 RPC。移除 JS 全量过滤分页和 N+1 confirmedMap 查询。下一步：**PERF-S1C**（合并在途+已确认到仓聚合，消除 confirmedMap N+1）。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-03 PERF-S1B DONE。Repository + Actions 接入 00027 三个 RPC，移除 JS 全量过滤分页和 N+1。全量 2639/2639（63 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。下一步 PERF-S1C。
...
2026-07-03 — PERF-S1B DONE。Repository + Actions 接入 00027 三个 RPC，移除 JS 全量过滤分页和 N+1。P3-S5B0~B5 全部完成。全量 2639/2639（63 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。下一步 PERF-S1C（合并在途+已确认到仓聚合）。
