...
**PERF-D-OVERVIEW 仓库概览服务端化**（2026-07-07 DONE，Migration 00032 已执行）

## Current Task

**PERF-D-OVERVIEW** — 同步页仓库概览改为服务端全量聚合（**DONE**，2026-07-07）。

`src/app/dashboard/sync/_components/sync-page-content.tsx` 原有 `warehouseOverview` useMemo，基于当前页 rows 做客户端聚合，只能反映当前分页数据，非全量最新状态。

**实现**：
- Migration 00032：新增 `get_sync_warehouse_overview()` RPC（SECURITY DEFINER，Admin 全部海外仓 / Operator 仅已分配仓库）— 已于 2026-07-07 在 Supabase SQL Editor 手动执行成功
- Repository 接口 + Mock + Supabase 实现
- Server Action `getSyncWarehouseOverview()`（authenticated only）
- `page.tsx` 通过 `Promise.all` 与分页查询并行获取概览，作为 prop 传入 `SyncPageContent`
- `sync-page-content.tsx` 删除 useMemo 聚合，直接使用 prop
- `perf-d-overview.test.ts`：26 项测试（Migration 契约 16 + Mock 行为 3 + 客户端源码 3 + 服务端源码 3）
- `src/types/database.ts`：添加 RPC 类型声明

**已完成**：
- PERF-D-CACHE：移除模块级进程缓存
- Phase D/E：分页 + 索引优化，Migration 00031 已执行
...
**Last Updated**: 2026-07-07 PERF-D-OVERVIEW Migration 00032 已执行确认（get_sync_warehouse_overview RPC 已在 Supabase SQL Editor 手动执行成功）。PERF-D-CACHE/PERF-D-OVERVIEW 已完成，Phase D/E Migration 已执行。P3-S1B 仍 BLOCKED_EXTERNAL（百世 API 权限未开通）。后续如重新生成 Supabase 类型，可替换当前手动添加的 database.ts RPC 类型声明。
