...
**P7-PRODUCT-OVERVIEW** — 可实施（2026-07-12 v4 合并整合：P7 与作战室合并为单一产品「全球库存总览」P7-A 基础总览 + P7-B 作战室增强层；解除 `BLOCKED_BY_DOMESTIC_INVENTORY`。P7-A 海外基础总览 + 国内占位可开工；P7-B 增强层待 P1 落盘后叠加；国内真实接入划归 P8，P7-C 启用国内补给判断）

## Current Task

**P6-OVERSEAS-PRODUCT-NAME-SIMPLIFY** — DONE（2026-07-10）。全量测试 3524/3524（0 failures）。海外库存表格产品名称列仅显示 BigSeller 原始品名，不再显示标准产品辅助信息。P7 暂不启动，国内库存方案暂不进行。
...
| P7-A | 全球库存基础总览（先上）：Product/Variant 一行、海外库存汇总、海外在途汇总、基础库存告警、国内占位、Admin/Operator warehouse_id 权限隔离；**不依赖 P1** | PLAN（解除 BLOCKED，可开工） |
| P7-B | 作战室增强层（后叠，依赖 P1）：复用 `forecast_stockout(...)` 算 earliest_stockout/urgency/分国 burn-down + 详情弹窗 + 后续 net_demand/suggest_qty/latest_order_date；与 P7-A 共用同一路由/列表 RPC/详情 RPC | PLAN（待 P1 落盘） |
...
| 新 Migration | 不新增（除 P1 Migration D 共享函数外） | 全部从已有表读取 |
| 新 RLS | 不新增 | 不建新表 |
...
| 分层依赖 | **P7-A 不依赖 P8 / 不依赖 P1**（海外基础总览 + 国内占位可先上）；**P7-B 依赖 P1** 的 `forecast_stockout(...)`；**国内补给判断依赖 P8**（P7-C 启用） | 解除 BLOCKED_BY_DOMESTIC_INVENTORY：海外先行、国内占位 |
...
**Current Task**: P6-OVERSEAS-PRODUCT-NAME-SIMPLIFY DONE（2026-07-10）。全量测试 3524/3524（0 failures）。海外库存产品名称列仅显示 BigSeller 原始品名，不再显示标准产品辅助信息。P7-MVP 解除阻塞（v4 合并整合 2026-07-12）：可实施 P7-A 海外基础总览 + 国内占位；P7-B 增强层待 P1 落盘后叠加；国内真实接入划归 P8，P7-C 启用国内补给判断。P6-OVERSEAS-INVENTORY-UX-V2 FINAL CLOSED（2026-07-09）。P3-S1B（百世 API 恢复）BLOCKED_EXTERNAL。
...
**Last Updated**: 2026-07-12 — **P7-V4-MERGE DONE**（P7 与作战室合并为单一产品「全球库存总览」）。P7-MVP 解除 BLOCKED_BY_DOMESTIC_INVENTORY：可实施 P7-A 海外基础总览 + 国内占位；P7-B 增强层待 P1 落盘后叠加；国内真实接入划归 P8，P7-C 启用国内补给判断。P6-OVERSEAS-INVENTORY-UX-V2 FINAL CLOSED（2026-07-09）。P3-S1B（百世 API 恢复）BLOCKED_EXTERNAL。
