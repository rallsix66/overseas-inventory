...
Phase 3 — 在途与物流（百世只读同步 + 手动补录 + 状态推进 + 入仓联动）

## Current Task

`P3-S5A` — 手动确认入仓事务与库存联动（**DONE**，2026-06-30）
...
- `P3-S3` — 手动创建/补录在途记录（2026-06-28 **DONE**，Codex 独立验收通过。表单页 `/dashboard/shipments/new` 就绪：船名/航次/起运港/目的港/国家/仓库/预计到仓日期/备注 + 可增删产品明细表格。`createShipment()` 使用 `requireActiveAuth()` + 仓库数据一致性校验（存在/启用/类型/国家一致）+ Operator 通过 `warehouseAccessRepository.canAccessWarehouse()` 校验分配权限。复用 `create_shipment_transactional` RPC（Migration 00005）。searchVariants 所有查询 error 检查 + notIn 归档过滤在 limit 前 + LIKE 转义 \\/%、_ + 独立 Repository 行为测试（mock createClient + variantRepository，不 mock shipmentRepository）+ 真实 Server Action 链路测试（Supabase 错误 → ShipmentError → ActionResult 中文错误）。目标测试 96/96，全量 1439/1439（44 文件），lint 0 errors / 26 warnings，build pass，git diff --check pass。未实现列表/详情/状态推进/入仓/库存联动。）
...
- `P3-S5A` — 手动确认入仓事务与库存联动（2026-06-30 **DONE**）。Migration 00023 新增 `warehouse_shipment_transactional` RPC（SECURITY INVOKER，Admin-only）：同事务内 SELECT shipment FOR UPDATE（校验存在/非 warehoused/有 warehouse_id/customs 状态）→ SELECT shipment_item FOR UPDATE（逐行 remaining > 0 超量保护）→ UPSERT inventory FOR UPDATE（quantity += remaining）→ UPDATE shipment_item.warehoused_quantity = quantity → UPDATE shipment.status = 'warehoused' → INSERT tracking_event。Repository `warehouseShipment()` 调用 RPC，DB/RLS 错误传递为 ShipmentError。Actions `warehouseShipment()` Admin-only + Zod 校验 + revalidatePath 列表与详情。UI：详情页 customs 状态显示「确认入仓」按钮（Dialog 二次确认 + 不可撤销警告 + 备注输入）；非 customs 状态显示不可入仓原因；Operator 不显示。90 项新测试（8 Zod + 28 Migration 源码检查 + 7 详情页源码 + 13 按钮组件源码 + 7 仓库行为 + 13 actions + 14 仓库/actions 源码检查）。全量 1778/1778 测试（51 文件，concurrency/best live 预存失败），lint 0 errors / 25 warnings（all pre-existing），build pass。Migration 00023 待用户在 Supabase SQL Editor 手动执行。）
...
**Current Task**: P3-S5A DONE（2026-06-30）。P3-S4A DONE（Migration 00022 已执行）。P3-S5 阶段 A 完成，阶段 B（P3-S5B）就绪。详情见下方 Next Step。
...
| 新 Migration（P3-S5A） | 00023（warehouse_shipment_transactional 确认入仓事务 RPC）待用户在 Supabase SQL Editor 手动执行 |
...
- 入仓库存联动待 P3-S5 实施（当前 P3-S4A 禁止手动推进 warehoused，入仓写入 inventory + shipment_item.warehoused_quantity 事务化在 P3-S5 完成）
...
- **P3-S2A**：DONE（2026-06-29，返工完成）。内部手动在途只读页面（`/dashboard/shipments` 列表 + `/dashboard/shipments/[id]` 详情），仅读内部 `shipment` / `shipment_item` / `tracking_event`，不读外部在途三表。
...
- **P3-S2**：依赖 P3-S1D，BLOCKED（完整在途列表与详情含百世双源展示）。P3-S2A 仅覆盖内部手动在途只读，百世数据双源展示仍需 P3-S1D 完成后实施。
- **P3-S3**：DONE（2026-06-28，Codex 独立验收通过）。
- **P3-S4**：依赖 P3-S2 + P3-S3，BLOCKED。
- **P3-S5**：依赖 P3-S4，BLOCKED。
- **P3-S6**：依赖 P3-S5，BLOCKED。

**Last Updated**: 2026-06-29 P3-S2A 返工完成。
...
**P5-SY12 特别关注阶段 B 核心文件（DONE / CODE COMPLETE）：**
...
| `src/features/inventory/repository.ts` | `getOverseasList()` 内部完成：归档过滤 → 关注标记 → 排序（关注置顶 + quantity 升序）→ 分页 |
| `src/features/inventory/actions.ts` | `getOverseasInventory()` 返回 `result.data` 含 `isFavorited` 字段（由 repository 层标记，不再单独返回 favoritedVariantIds） |
...
2026-06-30 — P3-S2C DONE。`shipmentRepository.getInTransitByVariant()` 按 variant_id 聚合在途数量（`quantity - warehoused_quantity`），排除 warehoused，Admin/Operator 仓库隔离。海外库存页新增"在途"/"库存+在途"列 + 在途统计卡片；Dashboard 关注产品动态新增"在途"列；Dashboard 在途库存卡片从占位替换为真实数据（SKU 数 + 在途总量）。14 项 repository 行为测试。全量 1541/1541 测试（46 文件），lint 0/26，build pass。不新增 Migration。P3-S1B CODE COMPLETE / BLOCKED_EXTERNAL。P3-S2/P3-S4/P3-S5/P3-S6 BLOCKED。
