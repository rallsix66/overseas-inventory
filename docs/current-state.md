...
`P5-SY10` — 自动 Dry Run 预审与后续自动化分阶段框架（IN_PROGRESS — P5-SY10A DONE，P5-SY10B DONE，P5-SY10C DONE，P5-SY10D DONE，P5-SY10E DONE。规则引擎 + 历史上下文提供器 + 自动预审编排 + 预审页面 UI + Vercel Cron 调度已实现。Sync 页面新增「自动预审」入口 + Cron Route Handler（API key 鉴权）支持每日自动预审。34 项 P5-SY10E 测试。715/715 TS 测试（23 文件），lint 0 errors，build pass。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。当前任务 P5-SY10F。）
...
- `P5-SY10E` — 调度机制：Vercel Cron Route Handler + 手动触发入口（2026-06-24 DONE。新增 `src/app/api/cron/dry-run/route.ts`：GET handler，CRON_API_KEY Bearer token 鉴权，调用 `runScheduledAutoPreReview()` 执行自动预审。新增 `vercel.json`：每日 UTC 01:00（北京时间 09:00）触发。`server-actions.ts` 新增 `runScheduledAutoPreReview()` 导出函数（API key 校验 + `_verifyBigSellerSessionCore` 无 auth 版 + 系统用户 ID）。`actions.ts` `triggerBatchDryRun`/`runAutoPreReview` 新增可选 `systemTriggeredBy` 参数（提供时跳过 requireActiveAdmin）。`.env.example` 新增 `CRON_API_KEY` / `CRON_SYSTEM_USER_ID`。34 项新测试（源码检查 + vercel.json 结构 + systemTriggeredBy 管线集成 + Real Write 隔离）。质量门：715/715 TS 测试（23 文件），lint 0 errors，build pass。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。仅触发 Dry Run + 规则评估，不调用 Real Write。）

## Awaiting Review

- 当前无待验收项目。P5-SY10F 为当前任务（独立验收与生产就绪）。P5-SY11 ProductVariant 软归档与库存视图降噪记入后置计划。
...
P5-SY10F — 独立验收与生产就绪（全量测试 + lint/build + Codex 独立审查 + 架构边界合规审查）。P5-SY11 软归档后置，当前不启动。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。
...
当前 P5-SY10 任务包（IN_PROGRESS — 任务包设计完成，P5-SY10A~F 子任务已拆分）。P5-SY9 全部子任务（A~K）DONE。

**P5-SY10 核心文件（规划）：**
- `src/features/sync/rules-engine.ts` — 规则引擎纯函数 `evaluateRules()`（新建）
- `src/features/sync/types.ts` — 新增 `RuleLevel` / `RuleEvaluation` / `RuleInput` / `RuleVerdict` / `WarehouseHistory` / `AutoPreReviewResult` 类型
- `src/features/sync/repository.ts` — SyncRepository 接口新增 `getWarehouseHistory()`
- `src/features/sync/supabase-repository.ts` — 实现 `getWarehouseHistory()`
- `src/features/sync/server-actions.ts` — 新增 `runAutoPreReview()` Server Action
- `src/app/dashboard/sync/_components/sync-page-content.tsx` — BatchReviewCard 扩展规则决策展示
- `src/app/api/cron/dry-run/route.ts` — Vercel Cron Route Handler（新建）
- `vercel.json` — cron 配置（新建或扩展）
...
2026-06-24（P5-SY10E DONE。Vercel Cron Route Handler + `vercel.json` 已创建。Cron 使用 CRON_API_KEY 鉴权 + CRON_SYSTEM_USER_ID 系统用户。`runScheduledAutoPreReview()` 导出函数 + `_verifyBigSellerSessionCore` 无 auth 版 + `systemTriggeredBy` 参数管线。`src/app/api/cron/dry-run/route.ts`：GET handler，Bearer token 401 保护。`vercel.json`：每日 UTC 01:00（北京时间 09:00）触发。34 项新测试。715/715 TS 测试（23 文件），lint 0 errors，build pass。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。仅触发 Dry Run + 规则评估，不调用 Real Write。当前任务 P5-SY10F。）
