...
- `P4-U3` — 修改用户角色（2026-07-01 **DONE**）。在 `UserDetailSheet` 详情 Sheet 中新增"修改角色"按钮。单击后弹出 `UserRoleChangeDialog` 确认对话框：选择新角色（过滤当前角色避免重复提交）、提交中显示 Loader2 pending 状态、失败展示中文错误。成功后关闭 Sheet 并 `router.refresh()` 刷新页面列表。复用现有 `updateUserRole` Server Action（Admin-only + 自降级保护 + 最后管理员保护），不新增 Migration。新增 `src/features/users/components/user-role-change-dialog.tsx`。`src/features/users/p4-u3.test.ts` — 35 项测试（架构合规 + 导入控制 + Dialog 行为 + Sheet 集成 + 透传 + 权限 + P4-U1/P4-U2 回归）。2095/2095 tests（55 文件，concurrency 与 best live 预存 env 依赖），lint 0 errors / 24 warnings（all pre-existing），build pass，git diff --check pass。不新增 Migration，不实现启用/禁用，不实现仓库分配。P4-U4 启用禁用已解除阻塞。
...
- **P4-U3**：DONE（2026-07-01）。修改角色功能完成。UserDetailSheet + UserRoleChangeDialog 确认对话框。复用现有 updateUserRole action（自降级 + 最后管理员保护）。35 项测试。2095/2095 tests。
...
- **P4-U4**（启用/禁用）：已解除阻塞（依赖 P4-U3 DONE）。可复用 `toggleUserActive` action + P4-U3 页面模式。

**Last Updated**: 2026-07-01 P4-U3 DONE。Phase 4 用户管理修改角色已完成；用户列表 + 角色变更已形成闭环。P4-U4 启用禁用已解除阻塞。
...
2026-07-01 — P4-U1 返工完成。Phase 4 用户管理数据层已收口；Phase 3 内部路径 S2A~S6 全完成；P3-S1B/P3-S1C/P3-S1D/百世路径仍因 API 授权阻塞。
