...
`P5-SY13A` — 仓库分配权限：权限基础与读路径收紧（REWORK DONE，2026-06-26，等待 Codex 复验）
...
- `P5-SY13A` — 仓库分配权限：权限基础与读路径收紧（2026-06-26 REWORK DONE，等待 Codex 复验。返工修复 3 项阻塞：1) `get_assigned_warehouse_ids()` 改为 GRANT EXECUTE TO authenticated（RLS 策略调用需要）；2) 移除 `accessibleWhIds.size > 0` 防误放守卫（空分配→空结果，不再返回全量）；3) `getSyncLogDetail()` 应用层补强 `canAccessWarehouse` 校验（serviceClient 绕过 RLS 的补偿）。Migration 00015 测试移至 `src/features/warehouse-access/00015-user-warehouses-migration.test.ts`（纳入 vitest 执行）。1101/1101 TS 测试（37 文件），lint 0 errors / 24 warnings，build pass。）
...
**Current Task**: P5-SY13A REWORK DONE（2026-06-26）。等待 Codex 复验，不自动进入 P5-SY13B。
...
| 日期 | 变更 |
|---|---|
| 2026-06-26 | **P5-SY13A REWORK DONE。返工修复 3 项阻塞：(1) get_assigned_warehouse_ids() GRANT EXECUTE TO authenticated；(2) 移除 accessibleWhIds.size > 0 防误放守卫（空分配→空结果）；(3) getSyncLogDetail() 应用层补强 canAccessWarehouse 校验。1101/1101 TS 测试（37 文件），Migration 00015 测试移至 src/ 纳入 vitest，lint 0 errors / 24 warnings，build pass。** |
...
无阻塞。P5-SY13A 已完成，等待用户确认下一任务。

## Next Step

P5-SY13A 已完成（2026-06-26）：仓库分配权限基础与读路径收紧。Migration 00015 新建 `user_warehouses` 表 + Seed 给 active operator 分配所有 active warehouse + 收紧 7 张表 RLS + 2 个 RPC operator 分支加入仓库分配过滤。应用层新增 `warehouseAccessRepository` + inventory/preferences/sync 按已分配仓库过滤 + toggleFavoriteAction 权限校验。不做管理 UI（P5-SY13B）。质量门：1048/1048 TS 测试（36 文件），lint 0 errors / 24 warnings，build pass。等待用户确认下一任务，不自动进入相邻任务。P5-SY10 Phase B 自动 Real Write 设计预留，当前不启用。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。
...
2026-06-26 — P5-SY13A REWORK DONE。返工修复 3 项阻塞：(1) get_assigned_warehouse_ids() GRANT EXECUTE TO authenticated（RLS 策略调用需要）；(2) 移除 accessibleWhIds.size > 0 防误放守卫（空分配→空结果）；(3) getSyncLogDetail() 应用层补强 canAccessWarehouse 校验（serviceClient 绕过 RLS 的补偿）。Migration 00015 测试已移至 src/features/warehouse-access/ 纳入 vitest 执行。质量门：1101/1101 TS 测试（37 文件），lint 0 errors / 24 pre-existing warnings，build pass。等待 Codex 复验。不自动进入 P5-SY13B。
