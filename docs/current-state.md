...
Phase 4 — 团队账号（管理员可查看和维护内部用户状态与角色）

## Current Task

`P4-U5` — 用户模块安全与流程验收（**DONE**，2026-07-01）
...
- `P4-U5` — 用户模块安全与流程验收（2026-07-01 **DONE**）。安全审计发现最后活跃管理员保护存在 TOCTOU 竞态条件：`updateUserRole` 和 `toggleUserActive` 的 countByRole 检查与数据库写入非原子，两个管理员可互相禁用/降级导致 0 管理员。**修复**：新增 Migration 00024（`update_user_role_protected` / `toggle_user_active_protected` RPC），使用 `pg_advisory_xact_lock` 序列化 Admin 写操作 + `FOR UPDATE` 行锁 + SECURITY INVOKER（不绕过 RLS）。Repository `updateRole()` / `toggleActive()` 改为调用 RPC，Actions 简化为 Auth + Zod + RPC 调用。Migration 静态契约测试 25 项。清理 `docs/current-state.md` Current Task References（P5-SY11/P5-SY12 → Phase 4 用户模块核心文件 + P4-U5 验收关注点）。2150/2150 tests（57 文件，concurrency 与 best live 预存 env 依赖），lint 0 errors / 24 warnings（all pre-existing），build pass，git diff --check pass。不新增业务功能，不实现仓库分配，不修改权限模型。Phase 4 用户管理全模块完成。
...
**Current Task**: P3-S6 DONE（2026-06-30）。P3-S5A DONE（Migration 00023 已执行并验证）。P3-S4A DONE（Migration 00022 已执行）。在途模块内部路径（S2A~S5A）全部完成，权限与 RLS 验收通过。P3-S5B（部分入仓/批量入仓）与 P3-S1B（百世 API 恢复）待后续任务。
...
**Last Updated**: 2026-07-01 P4-U5 DONE。Phase 4 用户模块安全与流程验收完成。最后活跃管理员保护 TOCTOU 竞态已通过 Migration 00024 RPC 修复。Phase 4 用户管理全模块（列表 + 角色变更 + 启用禁用 + 安全验收）完成闭环。
...
- **竞态修复**：Migration 00024 RPC 使用 `pg_advisory_xact_lock(987654321)` 序列化 Admin 写操作 + `FOR UPDATE` 锁定目标行，消除「两个管理员互相禁用/降级」TOCTOU 竞态窗口。
- **错误消息**：所有 RPC RAISE EXCEPTION 使用中文错误消息，Repository 捕获并通过 `UserError('DB_ERROR', error.message)` 传播至 ActionResult。

## Last Updated

2026-07-01 — P4-U5 DONE。Phase 4 用户模块安全与流程验收完成。关键发现：最后活跃管理员保护存在 TOCTOU 竞态，已通过 Migration 00024 RPC（pg_advisory_xact_lock + SECURITY INVOKER）修复。用户管理 CRUD + 权限链 + 竞态修复已形成完整闭环。
