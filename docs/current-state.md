...
`LOW-STOCK-PAGINATION` — `getLowStock()` RPC 分页补齐（**DONE**，2026-07-04，返修完成）。

LOW-STOCK-PAGINATION 验收结论（含 RPC 返修）：
- 全量 2754/2754（65 文件），build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有），git diff --check pass
- **Migration 00028**：新增 `get_low_stock` RPC（SECURITY INVOKER，SQL 层完成归档排除、仓库隔离、quantity <= safety_stock 过滤、gap 计算、ORDER BY gap DESC, quantity ASC、LIMIT p_limit）
- `inventoryRepository.getLowStock()` 改为调用 `supabase.rpc('get_low_stock')`，移除 JS 层全量查询/归档/仓库过滤
- 移除 `getUserArchivedVariantIds` 辅助函数和 `warehouseAccessRepository` 导入（均已下沉 RPC）
- Dashboard 调用方使用 `getLowStock({ userId: user.id })`，其他行为不变
- 返回类型保持 `Promise<InventoryItem[]>`（非 PaginatedResult）

| 子任务 | 内容 | 状态 |
|--------|------|------|
| Migration | 00028：get_low_stock RPC（SQL 层过滤+排序+limit） | DONE |
| repository | getLowStock → supabase.rpc('get_low_stock') + mapOverseasRow，移除 JS 过滤 | DONE |
| database.ts | 新增 get_low_stock 函数类型 | DONE |
| 测试 | low-stock-pagination-migration.test.ts（25 项）+ p2-d2-low-stock-summary.test.ts 更新 + p5-sy11g-d / p5-sy12 / p5-sy13a 适配 | DONE |
| 文档 | current-state.md / current-task.md / Technical Debt 更新 | DONE |

下一步：getLowStock() 分页补齐完成（RPC 正确实现）。UNMATCHED-PAGINATION、LOW-STOCK-PAGINATION 均已完成。P3-S1B 仍 BLOCKED_EXTERNAL。可推进其他未阻塞任务。
...
| 新 Migration（LOW-STOCK-PAGINATION） | 00028（get_low_stock RPC）待手动执行 |
...
**Last Updated**: 2026-07-04 LOW-STOCK-PAGINATION DONE（RPC 返修完成）。getLowStock() Migration 00028 RPC 补齐。PERF-S1 全系列、P4-UX、P2-D2、SIDEBAR-ENABLE、UNMATCHED-PAGINATION、LOW-STOCK-PAGINATION 均已完成。全量 2754/2754（65 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL，可推进其他未阻塞任务。
