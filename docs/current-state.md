...
- `LowStockSummarySection`：新增 Client Component，按仓库分组展示低库存项（SKU/产品/库存/安全库存/缺口），缺口大优先，库存低优先
- `DashboardPage`：新增 `getLowStock()` 调用 + gap 计算 + 排序，失败不崩溃 Dashboard
- 不依赖用户关注（关注产品 vs 低库存汇总各自独立 try/catch）
- SKU 链接跳转海外库存搜索页；"查看全部" → 低库存筛选页

| 子任务 | 内容 | 状态 |
|--------|------|------|
| P2-D2 组件 | LowStockSummarySection：仓库分组 + 空状态/错误状态 + MAX_DISPLAY=15 + "还有 N 项" | DONE |
| P2-D2 数据 | Dashboard page.tsx getLowStock + gap/sort + try/catch 隔离 | DONE |
| P2-D2 测试 | p2-d2-low-stock-summary.test.ts 43 项测试（数据链路/组件结构/空错误状态/仓库分组/SKU 链接/关注隔离/架构合规/P5-SY12D 回归） | DONE |
| P2-D2 文档 | current-state.md / current-task.md / phase-2-inventory-dashboard.md 同步 | DONE |

下一步：P2-D2 完成。P3-S1B 仍 BLOCKED_EXTERNAL。可推进其他未阻塞任务。
...
- **P2-D2**（DONE，2026-07-04）：Dashboard 低库存汇总看板。新增 LowStockSummarySection 组件（仓库分组 + 空/错误状态 + SKU 跳转）+ Dashboard page.tsx getLowStock 数据接入。全量 2703/2703（64 文件），lint 仅既有 smoke-test-00025.ts 5e/25w，build pass。不新增 Migration/RLS/权限变更。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-04 P2-D2 DONE。Dashboard 低库存汇总看板完成。PERF-S1 全系列、P4-UX、P2-D2 均已完成。全量 2703/2703（64 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL，可推进其他未阻塞任务。
