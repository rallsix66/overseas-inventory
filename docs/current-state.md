...
`PERF-S1B` — Repository / Server Action 接入 RPC（**DONE**，2026-07-03，Migration 00027 已执行并通过数据库侧 smoke 验证）。P3-S5B 全部完成（B0~B5 ✅）。海外库存列表/统计/在途+已确认聚合已接入 00027 三个 RPC（`get_overseas_inventory` / `get_overseas_stats` / `get_in_transit_confirmed_aggregate`），移除 JS 全量过滤分页和 N+1 查询。PERF-S1C 的聚合内容（合并在途 + 已确认到仓聚合、消除 `getConfirmedWarehousedByWarehouse` 按仓循环查询）已并入 PERF-S1B 完成。下一步：PERF-S1D（关键按钮局部更新，减少 router.refresh()）或 PERF-S1E（质量门/文档收口），以设计文档最终调整为准。
...
- **PERF-S1B**（DONE，2026-07-03）：Repository（getOverseasList/getOverseasStats/getInTransitConfirmedAggregate）+ Actions（getOverseasInventory）接入 00027 三个 RPC。移除 JS 全量过滤分页和 N+1 confirmedMap 查询。PERF-S1C 的聚合内容（合并在途+已确认到仓聚合、消除按仓循环查询）已并入本任务完成。下一步：**PERF-S1D**（关键按钮局部更新，减少 router.refresh()）或 **PERF-S1E**（质量门/文档收口）。
- **P3-S5B**：全部完成（B0~B5 ✅）。
...
**Last Updated**: 2026-07-03 PERF-S1B DONE。Repository + Actions 接入 00027 三个 RPC，移除 JS 全量过滤分页和 N+1。全量 2639/2639（63 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。PERF-S1C 已并入 PERF-S1B；下一步 PERF-S1D 或 PERF-S1E。
...
2026-07-03 — PERF-S1B DONE。Repository + Actions 接入 00027 三个 RPC，移除 JS 全量过滤分页和 N+1。P3-S5B0~B5 全部完成。全量 2639/2639（63 文件），lint 5 errors / 26 warnings（all pre-existing），build pass。PERF-S1C 已并入 PERF-S1B；下一步 PERF-S1D 或 PERF-S1E。
