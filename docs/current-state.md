...
`SIDEBAR-ENABLE` — 侧边栏 SKU 管理/待处理 SKU 入口启用（**DONE**，2026-07-04）。

SIDEBAR-ENABLE 验收结论：
- 全量 2707/2707（64 文件），build pass，lint 5 errors / 25 warnings（仅 `smoke-test-00025.ts` 既有），git diff --check pass
- `sidebar-nav.tsx`：`/dashboard/variants` 和 `/dashboard/variants/unmatched` 的 phase 从 `'1'` 改为 `'0'`，入口已启用
- 页面和功能由 P5-SY11E/P5-SY11G 已完成，本任务仅启用导航入口 + 文档收口
- Current Implementation Limits 已修正：删除"SKU 管理与待处理 SKU 仍灰显"

| 子任务 | 内容 | 状态 |
|--------|------|------|
| sidebar-nav 修改 | phase '1' → '0' for variants + variants/unmatched | DONE |
| 测试 | p5-sy11g-e-ui.test.ts 新增 4 项 sidebar-nav 断言（phase 0 / 无 phase 1 / domestic 保持 phase 2） | DONE |
| 文档 | current-state.md / current-task.md / Current Implementation Limits 修正 | DONE |

下一步：侧边栏入口启用完成。P3-S1B 仍 BLOCKED_EXTERNAL。可推进其他未阻塞任务。
...
**Last Updated**: 2026-07-04 SIDEBAR-ENABLE DONE。侧边栏 SKU 管理/待处理 SKU 入口已启用。PERF-S1 全系列、P4-UX、P2-D2、SIDEBAR-ENABLE 均已完成。全量 2707/2707（64 文件），lint 仅 smoke-test-00025.ts 既有 5 errors / 25 warnings，build pass。P3-S1B 仍 BLOCKED_EXTERNAL，可推进其他未阻塞任务。
