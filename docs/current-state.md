...
2026-07-01 — P4-U5 返工完成。Migration 00025 修复 RPC 直接调用权限缺口：auth.uid() 身份绑定 + REVOKE/GRANT 权限加固 + check_operator_profile_update 触发器禁止 operator 自升 role_id/is_active。用户管理 CRUD + 权限链 + 竞态修复 + RPC 调用者身份绑定已形成完整闭环。
