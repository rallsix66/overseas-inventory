...
`P5-SY9` — 海外仓库存同步生产化（IN_PROGRESS — P5-SY9A~G DONE；P5-SY9G AWAITING_REVIEW；P5-SY9H~I PENDING。Web 真实写入入口由 WEBSYNC_REAL_WRITE_ENABLED feature gate 保持 disabled，P5-SY9I 完成并通过 Codex 验收前不启用。）
...
P5-SY9G AWAITING_REVIEW：待 Codex 独立验收。通过后进入 P5-SY9H（页面体验与运营可用性收口）。Web 真实写入不得上线。
