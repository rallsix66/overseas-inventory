...
| **P3-S5B4** | 批量 UI + 海外库存"已确认到仓"列 | P3-S5B3 | **DONE (2026-07-02)** + **返修完成 (2026-07-02)** | 批量确认到仓 UI（/dashboard/shipments/batch 路由 + BatchWarehousePage 客户端组件：checkbox 选择/全选/展开行加载明细/数量校验/全额确认/批量提交结果汇总）+ 海外库存页新增"已确认到仓"列（confirmedMap，口径仅 customs 或 warehoused + bigseller_absorbed_at IS NULL）+ 侧边栏"批量入仓"Admin-only 入口 + listEligibleForBatchWarehousingAction Server Action。**返修**：listEligibleForBatchWarehousingAction 新增 eligibleShipmentFiltersSchema Zod 校验（country/warehouseId/page/pageSize）+ safeParse 失败返回中文错误不进入 repository；海外库存展开行 colSpan 14→13（修复列数计数错误）。新增 70 项测试。全量 2452/2452（61 文件），lint 5/26（all pre-existing），build pass。 |
| **P3-S5B5** | 应用行为测试 + 文档同步 + 质量门 | P3-S5B4 | **DONE (2026-07-03)** | 新增 `p3-s5b5-behavior.test.ts` 108 项应用行为测试，覆盖：详情页双模式 warehouseBlockReason/PartialWarehouseEntry/BigsellerAbsorptionButton、listEligibleForBatchWarehousingAction mock 行为、validateEntry 校验分支、confirmedMap 构建与单仓失败隔离、Actions 权限链与安全边界、批量入仓页边界状态、eligibleShipmentFiltersSchema Zod 校验、海外库存页列数一致性、权限审计矩阵。全量 2561/2561 测试（62 文件），lint 5/26（all pre-existing），build pass。P3-S5B0~B5 全部完成。 |
| **P3-S5** | 入仓事务与库存联动 | P3-S4 | **DONE** | P3-S5A（全部入仓）DONE。P3-S5B（部分/批量入仓）B0~B5 全部完成。当前业务口径：DIS 入仓是运营跟踪工具，inventory.quantity 唯一事实来源是 BigSeller 同步。 |
...
> **注意**：P3-S3 最初设计时允许 Admin 与 Operator 均可创建在途记录（Operator 仅已分配仓库）。**P3-S2E（2026-06-30）已将创建/编辑/状态推进全部收紧为 Admin-only**。P3-S6 权限审计确认当前 Action 层全部写操作拒绝 Operator。Repository 内 Operator 仓库校验分支保留为防御性代码，不做功能依赖。
...
**当前任务**：P3-S5B 全部完成（P3-S5B0~B5 ✅，2026-07-03），下一模块待用户确认
...
**P3-S5B1 已完成**（2026-07-02 + 返修完成）：Migration 00026（`bigseller_absorbed_at` 列 + `partial_warehouse_shipment` RPC）+ database.ts 类型同步 + feature types/schema + 93 项静态契约测试。返修：jsonb_typeof + 正则预检输入加固（防止 PG 英文 cast 错误泄漏）。Migration 00026 已于 2026-07-02 手动执行并验证通过。
...
**P3-S4A 状态**：DONE（2026-06-30，Migration 00022 已执行并验证）。
...
**下一步**：P3-S5B 全部完成。P3-S5B5 DONE（2026-07-03）。P3-S5B4 DONE + 返修完成。P3-S5B3 DONE + 收口返修。P3-S5B2 DONE + 返修完成。P3-S5B1 DONE。P3-S5B0 DONE。Phase 3 内部路径（S2A~S6）全部 DONE。P3-S5B 拆分：B0 ✅ B1 ✅ B2 ✅ B3 ✅ B4 ✅ B5 ✅。P3-S4（百世路径）依赖 P3-S1B 解除阻塞。P3-S1B（百世 API 恢复）待百世开通 API 权限。下一模块待用户确认。
