...
| **P3-S5A** | 手动确认入仓事务与库存联动 | P3-S4A | **DONE (2026-06-30)** | Migration 00023 新增 `warehouse_shipment_transactional` RPC（Admin-only + FOR UPDATE + 超量保护 + UPSERT inventory + 同事务 atomic）。Repository `warehouseShipment()` → Actions `warehouseShipment()` Admin-only + Zod → UI `WarehouseShipmentButton` Dialog 二次确认。90 项新测试。1778/1778 测试（51 文件），lint 0/25，build pass。Migration 00023 待手动执行。不做百世映射/批量入仓/部分入仓。 |
