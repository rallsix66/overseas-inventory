...
| **P3-S5B5** | 应用行为测试 + 文档同步 + 质量门 | P3-S5B4 | **DONE (2026-07-03)** | 新增 `p3-s5b5-behavior.test.ts` 108 项应用行为测试：详情页双模式分支覆盖 + listEligibleForBatchWarehousingAction mock 行为 9 + validateEntry 9 + confirmedMap 单仓失败隔离 12 + 安全边界 30 + 边界状态 12 + Zod schema 10 + 权限审计 6。全量 2561/2561 测试（62 文件），lint 5/26（all pre-existing），build pass。P3-S5B0~B5 全部完成。 |
