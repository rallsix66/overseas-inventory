...
| **P3-S5B2** | Repository + Server Actions | P3-S5B1 | **DONE (2026-07-02)** | Repository 5 方法（partialWarehouse/listEligibleForBatchWarehousing/getConfirmedWarehousedQuantity/getConfirmedWarehousedByWarehouse/confirmBigsellerAbsorption）+ Actions 3 函数（partialWarehouseShipment/batchWarehouseShipments/confirmBigsellerAbsorption）。snake_case→camelCase 映射。79 项测试。不写 inventory，不调 00023。全量 2359/2359 测试（60 文件），lint 5/26，build pass。 |
| **P3-S5B3** | 详情页双模式按钮 + PartialWarehouseDialog | P3-S5B2 | **待开始** | 详情页恢复确认到仓按钮（双模式：全额/部分）+ PartialWarehouseDialog + BigsellerAbsorptionButton |
...
**当前任务**：`P3-S5B3` — 详情页双模式按钮 + PartialWarehouseDialog + BigsellerAbsorptionButton（**待开始**，2026-07-02）

**P3-S5B2 已完成**（2026-07-02）：Repository 新增 5 个方法（partialWarehouse/listEligibleForBatchWarehousing/getConfirmedWarehousedQuantity/getConfirmedWarehousedByWarehouse/confirmBigsellerAbsorption）+ Actions 新增 3 个 Server Action（partialWarehouseShipment/batchWarehouseShipments/confirmBigsellerAbsorption）。snake_case→camelCase 映射 + Admin-only + Zod + ShipmentError 中文错误传播。79 项测试（9 组）。不写 inventory.quantity，不调 00023 RPC。全量 2359/2359 测试（60 文件），lint 5/26，build pass。
...
**下一步**：P3-S5B3（详情页双模式按钮 + PartialWarehouseDialog + BigsellerAbsorptionButton）待开始。P3-S5B2 DONE（Repository + Actions + 79 项测试）。P3-S5B1 DONE（Migration 00026 + types/schema + 93 项静态测试，Migration 已执行）。P3-S5B0 DONE（旧 00023 应用层入口已封存）。Phase 3 内部路径（S2A~S6）全部 DONE。P3-S5B 拆分：B0 ✅ B1 ✅ B2 ✅ B3 待开始 B4~B5 待后续。P3-S4（百世路径）依赖 P3-S1B 解除阻塞。P3-S1B（百世 API 恢复）待百世开通 API 权限。
