...
| **P3-S2D** | 在途库存聚合精确到仓库 | P3-S2C | **DONE (2026-06-30)** | `shipmentRepository.getInTransitByVariantAndWarehouse()` 按 (variant_id, warehouse_id) 聚合在途数量。海外库存页每行 inTransitQuantity 精确匹配 variantId + warehouseId（防串仓）。Dashboard 保持 variant 总在途。16 项 repository 行为测试含跨仓隔离验证。1558/1559 测试（49 文件），lint 0 errors / 27 warnings，build pass。不新增 Migration。 |
