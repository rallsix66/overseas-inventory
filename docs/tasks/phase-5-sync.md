...
| P5-SY11 | ProductVariant 软归档与库存视图降噪 | P5-SY10 全部子任务（A~F）DONE | IN_PROGRESS（P5-SY11A~F 子任务已拆分。Migration 00011 增加 is_archived + archived_at + archived_by + 部分索引 + RLS 收紧。Repository 新增 archive/restore。Server Actions requireActiveAdmin()。Inventory 默认过滤已归档。Variant 页面实现列表+归档UI。同步写入链路不受影响。等待 Codex 独立设计审查后启动 P5-SY11A。） |
...
## P5-SY11 ProductVariant 软归档与库存视图降噪（IN_PROGRESS）

P5-SY11 是运营降噪任务，依赖 P5-SY10 全部子任务完成并通过 Codex 独立验收。当前任务包设计完成，等待 Codex 独立设计审查后启动实现。

### 子任务拆分

| Sub-Task ID | 任务 | 目标 | 依赖 | 状态 |
|---|---|---|---|---|
| **P5-SY11A** | Migration 00011：`is_archived` 列 + 审计字段 + 索引 + RLS | 新增 migration，含 `is_archived`（默认 false）+ `archived_at` + `archived_by`（FK→profiles）+ 部分索引 + RLS 收紧（Operator SELECT `AND is_archived=false`） | — | PENDING |
| **P5-SY11B** | 类型同步 + Repository：archive/restore/filter | 更新 database.ts 类型；variantRepository 新增 archive/restore；list/getUnmatched 增加 `archiveStatus` 过滤（默认 active）；match/unmatch/batchMatch 阻止已归档操作 | P5-SY11A | PENDING |
| **P5-SY11C** | Server Actions：archiveVariants / restoreVariants | `requireActiveAdmin()`，Zod 校验 + revalidatePath | P5-SY11B | PENDING |
| **P5-SY11D** | Inventory 层过滤：默认视图隐藏已归档 | getOverseasList/getLowStock/getOverseasStats 过滤已归档；getByProductId 不过滤 | P5-SY11A | PENDING |
| **P5-SY11E** | Variant 列表页面 + 归档/恢复 UI | 实现 variants/page.tsx（表格 + 归档筛选标签 + Admin 批量归档/恢复）+ unmatched/page.tsx（仅活跃）；Operator 只读 | P5-SY11C, P5-SY11D | PENDING |
| **P5-SY11F** | 同步非回归 + 质量门 + 文档收口 | 全量测试 + lint/build + Python + Codex 独立审查 | P5-SY11E | PENDING |
