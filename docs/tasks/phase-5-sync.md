...
| **P5-SY13A** | **仓库分配权限：权限基础与读路径收紧。Migration 00015 + warehouseAccessRepository + inventory/preferences/sync 读路径收紧 + toggleFavoriteAction 权限校验。返工修复 3 项阻塞：(1) get_assigned_warehouse_ids() GRANT EXECUTE；(2) 移除 size > 0 防误放守卫；(3) getSyncLogDetail() 补强 canAccessWarehouse。不做管理 UI（P5-SY13B）。** | P5-SY12D | **REWORK DONE**（2026-06-26。1101/1101 TS 测试 + lint 0 errors + build pass。等待 Codex 复验。） |
