...
| **P5-SY10B** | 历史上下文提供器：基线追踪 + 连续失败检测 | 实现 `getWarehouseHistory()` 从 sync_run 推导历史基线（仅 dry_run failed，real_write 屏障）；走 Repository 接口 | P5-SY10A | **返工完成待 Codex 复验**（2026-06-24。35 项测试。621/621 TS，lint 0 errors，build pass） |
