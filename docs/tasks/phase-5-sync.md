...
| **P5-SY12C** | **特别关注阶段 C 动态告警：Migration 00014（inventory.daily_sales/estimated_days + warehouse.lead_time_days + RPC 扩展）；Python sync 链路透传校验；FollowedVariantBasic 扩展 5 字段；getFollowedVariantsBasic 动态告警规则（estimated_days < lead_time_days→critical，quantity < safety_stock→warning）；Dashboard 日销/可售天数/补货周期列 + alertLevel badge。RUNTIME VERIFIED 2026-06-26：Migration 00014 手动执行成功，5 仓 lead_time_days=30，daily_sales 全部写入，estimated_days 按 BigSeller 可用数据写入（部分 SKU 为空属正常），生产验证通过。** | P5-SY12 | **DONE + RUNTIME VERIFIED**（2026-06-26。1006/1006 TS 测试 + lint 0 errors + build pass + Python compileall pass + 生产验证通过。） |
| **P5-SY12D** | **特别关注阶段 D 运营可用性收口：新增 FollowedProductsSection Client Component — 状态筛选（全部/紧急/低库存/正常/数据不足）+ 每行跳转海外库存 SKU 搜索 + 未匹配 SKU tooltip 提示（不参与 safety_stock 判断，仍可 estimated_days 告警）+ 边界状态（筛选无结果/加载失败/空关注/全未匹配）。不改数据模型/Migration。** | P5-SY12C | **DONE**（2026-06-26。1014/1014 TS 测试 + lint 0 errors + build pass。） |
| **P5-SY13A** | **仓库分配权限：权限基础与读路径收紧。Migration 00015 + warehouseAccessRepository + inventory/preferences/sync 读路径收紧 + toggleFavoriteAction 权限校验。返工修复 3 项阻塞：(1) get_assigned_warehouse_ids() GRANT EXECUTE；(2) 移除 size > 0 防误放守卫；(3) getSyncLogDetail() 补强 canAccessWarehouse。不做管理 UI（P5-SY13B）。** | P5-SY12D | **DONE**（2026-06-26。Production migration verified：00015 已执行，user_warehouse_assignments=0 当前单用户/Admin 场景可接受。1101/1101 TS 测试 + lint 0 errors + build pass。下一步：P5-SY13B。） |
| **P5-SY13B** | **仓库分配管理 UI：Admin 管理 operator 仓库分配。Migration 00016 新增 `update_user_warehouses` RPC（SECURITY DEFINER + admin/operator/overseas 三重校验 + DISTINCT 去重 + 事务性 DELETE/INSERT + REVOKE/GRANT 授权）。Repository 先校验再通过 RPC 原子写入。返工 Round 1（业务校验 + 事务性 RPC）+ Round 2（RPC 授权 + 测试纳入 Vitest）。** | P5-SY13A | **DONE**（2026-06-26。production migration verified：rpc_exists=true / authenticated_can_execute=true / anon_can_execute=false。1199/1199 TS 测试 + lint 0 errors + build pass。） |
...
P5-SY11A~F 按全局 ProductVariant 状态实现软归档（技术实现完成）。P5-SY11G 语义返工将归档从全局 `is_archived` 迁移为用户级 `user_variant_preference` 偏好表。所有用户均可归档/恢复，每人独立视图，A 的归档不影响 B。00011 历史文件保持不可变；遗留列曾停止业务读写，最终由 OPT-4 的 00048 前向清理。
...
> **语义变更说明**：上表中 P5-SY11A~F 的原始规格描述（`requireActiveAdmin()`、Admin 专属归档、Operator 只读、全局 `is_archived`）已被 P5-SY11G 最终语义替换：所有登录用户（Admin + Operator）均可归档/恢复，使用 `requireActiveAuth()`，归档为 `user_variant_preference` 个人偏好（非全局），每人独立视图，A 的归档不影响 B。

### P5-SY11G 语义返工目标边界（DONE — 2026-06-25）

- 废弃 `product_variant.is_archived` 全局列（不删除或回写 Migration 00011；先停止业务代码读写，最终由 00048 前向清理）。
...
- `product_variant.is_archived` 曾作为遗留列停止读写；00011 保持不可变，最终列状态由 00048 清理。
