...
| P5-SY11 | ProductVariant 软归档与库存视图降噪 | P5-SY10 全部子任务（A~F）DONE | **SEMANTIC REWORK PENDING** — P5-SY11A~F 技术实现完成（914/914 TS + lint 0 + build pass + Python 271），但业务语义冲突需返工（全局 is_archived → 用户级偏好）。P5-SY11G（P5-SY11-REWORK）PENDING。 |
| P5-SY11G | 语义返工：用户级 Variant 归档偏好（user_variant_preference 表） | P5-SY11A~F | **PENDING** — 任务包已创建，等待 Codex 审查。 |
...
## P5-SY11 ProductVariant 软归档与库存视图降噪（P5-SY11A~F DONE，P5-SY11G SEMANTIC REWORK PENDING）

P5-SY11A~F 按全局 ProductVariant 状态实现软归档（`product_variant.is_archived` 列 + Admin 专用操作），技术实现完成但用户已确认业务语义不一致：归档应为**每个用户自己的个人偏好**，非全局 Variant 状态。P5-SY11G（P5-SY11-REWORK）将归档从全局状态迁移为 `user_variant_preference` 用户级偏好表，所有用户均可归档/恢复，每人独立视图。详见 `docs/tasks/current-task.md`。

### P5-SY11A~F 技术实现（已完成，代码可复用）

P5-SY11 是运营降噪任务，依赖 P5-SY10 全部子任务完成并通过 Codex 独立验收。P5-SY11A~F 全部 DONE（2026-06-25）。22 项非回归测试，914/914 TS + lint 0 + build pass + Python 271。
...
| **P5-SY11G** | **语义返工：用户级 Variant 归档偏好** | 新建 `user_variant_preference` 表（Migration 00012），废弃全局 `is_archived`；所有用户均可归档/恢复；每人独立视图；预留"特别关注"扩展 | P5-SY11F | **PENDING** — 任务包已创建，等待 Codex 审查 |

### P5-SY11G 语义返工目标边界（PENDING）
