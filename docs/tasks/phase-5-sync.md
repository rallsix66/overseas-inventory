...
| **P5-SY10D** | 预审页面 UI：自动预审入口 + 规则决策徽标 + 可展开规则详情 | Sync 页面新增「自动预审」按钮。BatchReviewCard 新增 RuleBadge + 可展开规则详情 + 复选框返工：独立 `autoReviewSelectedItems` Set，PASS/WARN 可选，BLOCK 不可选。29 项新测试。681/681 TS。 | P5-SY10C | AWAITING_REVIEW |
| **P5-SY10E** | 调度机制：Vercel Cron Route Handler + 手动触发入口 | `src/app/api/cron/dry-run/route.ts` + `vercel.json` | P5-SY10D | PENDING |
