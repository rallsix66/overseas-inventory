...
`P5-SY11E DONE` — Variant 列表页面 + 归档/恢复 UI 完成（853/853 测试，lint 0 errors，build pass）。P5-SY11F PENDING，下一步 P5-SY11F。P5-SY11D DONE — Inventory 层归档过滤完成。P5-SY11C DONE — Server Actions 完成。P5-SY11B DONE — Repository 软归档能力完成。P5-SY9 全部子任务（A~K）DONE。P5-SY10 全部子任务（A~F）DONE。
...
| **P5-SY11E** | Variant 列表页面 + 归档/恢复 UI | 实现 `variants/page.tsx`（Server + Client Component）：数据表格、归档筛选标签（活跃/已归档/全部）、Admin 批量归档/恢复按钮；`unmatched/page.tsx` 仅显示活跃未匹配；Operator 只读 | P5-SY11C, P5-SY11D | **DONE**（2026-06-25。853/853 测试，lint 0 errors，build pass） |
