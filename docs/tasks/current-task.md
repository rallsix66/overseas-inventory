...
`IN_PROGRESS` — P5-SY10A DONE（规则引擎核心），P5-SY10B DONE（历史上下文提供器）。P5-SY10C~F 待启动。P5-SY9 全部子任务（A~K）DONE。
...
| **P5-SY10B** | 历史上下文提供器：基线追踪 + 连续失败检测 | 实现 `getWarehouseHistory()` 从 sync_run + sync_log 推导 hasBaseline / consecutiveFailures / lastSuccess / stats。走 Repository 接口。 | P5-SY10A | **DONE**（2026-06-24。`SyncRepository.getWarehouseHistory()` + MockRepository + SupabaseSyncRepository 实现。`buildResultSummary()` 持久化 scraperMeta。31 项测试。617/617 TS，lint 0，build pass。） |
