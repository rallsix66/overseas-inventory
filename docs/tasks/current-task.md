...
`DONE` — P5-SY10A DONE（规则引擎核心），P5-SY10B DONE（历史上下文提供器），P5-SY10C DONE（自动预审编排），P5-SY10D DONE（预审页面 UI，Codex 验收通过），P5-SY10E REWORK DONE（2026-06-24 返工：发现 Cron 路径无法使用 claim_sync_run（要求 auth.uid()），新增 Migration 00010 claim_sync_run_system RPC（service_role only，仅 dry_run，校验系统用户为激活 admin）、Repository 新增 claimSyncRunSystem、SyncService 新增 _systemClaimConfig、移除 actions.ts 中 systemTriggeredBy 绕权参数、server-actions.ts runScheduledAutoPreReview 直接构造系统 SyncService 不经过 createSyncActions。744/744 TS 测试，lint 0 errors，build pass，253 Python 通过）。P5-SY10F DONE（2026-06-24 文档收口完成，Codex 独立验收通过）。P5-SY9 全部子任务（A~K）DONE。
...
| **P5-SY10B** | 历史上下文提供器：基线追踪 + 连续失败检测 | 实现 `getWarehouseHistory()` 从 sync_run 推导 hasBaseline / consecutiveFailures（仅统计 mode='dry_run' 且 status='failed'，real_write/completed/in_progress 均作为屏障）/ lastSuccess / stats。走 Repository 接口。 | P5-SY10A | **DONE**（2026-06-24，Codex 返工复验通过。35 项测试含 real_write 屏障验证。621/621 TS，lint 0 errors，build pass。） |
| **P5-SY10C** | 自动预审编排：Server Action 串联 health → 逐仓预取历史 → batch Dry Run → rule eval | **DONE**（2026-06-24，Codex 独立复验通过。返工修正：① 在 triggerBatchDryRun 之前逐仓预取 getWarehouseHistory，缓存到 Map，保证 cold start 仓库 hasBaseline=false；② history 获取失败 → BLOCK（history_unavailable），不 fallback 冷启动；③ 新增冷启动 R7/R11 路径测试；④ 新增预取历史排他性测试。30 项测试。651/651 TS 测试，lint 0 errors，build pass。WEBSYNC_REAL_WRITE_ENABLED 仍 disabled。） | P5-SY10B | DONE |
| **P5-SY10D** | 预审页面 UI：自动预审入口 + 规则决策徽标 + 可展开规则详情 | Sync 页面新增「自动预审」按钮（调用 `runAutoPreReview()`）。BatchReviewCard 扩展 `ruleVerdict` prop：RuleBadge（PASS 绿/WARN 黄/BLOCK 红）+ 可展开规则详情（evaluations[].message）+ 复选框：独立 `autoReviewSelectedItems` Set + PASS/WARN + status=ready 可选 / BLOCK/failed/blocked 不可选。统计栏 PASS/WARN/BLOCK 计数。保留「批量 Dry Run」按钮。Operator 只读。清理 unused imports。30 项新测试（含 8 项复选框行为源码检查）。Codex 独立验收通过。 | P5-SY10C | DONE |
| **P5-SY10E** | 调度机制：Vercel Cron Route Handler + 手动触发入口 | **REWORK DONE**（2026-06-24 返工。发现 Cron 路径无法使用 claim_sync_run（要求 auth.uid()）。新增 Migration 00010 claim_sync_run_system RPC（service_role only，仅 dry_run，校验系统用户为激活 admin）→ Repository 新增 claimSyncRunSystem → SyncService 新增 _systemClaimConfig → 移除 actions.ts systemTriggeredBy 绕权参数（手动 triggerBatchDryRun/runAutoPreReview 始终调用 requireActiveAdmin）→ server-actions.ts runScheduledAutoPreReview 直接构造带 _systemClaimConfig 的 SyncService，不经过 createSyncActions。`.env.example`：新增 CRON_API_KEY / CRON_SYSTEM_USER_ID。744/744 TS（23 文件），lint 0 errors，build pass，253 Python 通过。仅触发 Dry Run + 规则评估，不调用 Real Write。） | P5-SY10D | REWORK DONE |
| **P5-SY10F** | 独立验收与生产就绪 | 全量测试（TS + Python）+ lint/build + Codex 独立审查。架构边界合规审查。生产启用文档。 | P5-SY10E | **DONE**（2026-06-24。文档收口完成：修正 P5-SY10E 表格行旧 systemTriggeredBy 描述、vercel.json cron 示例时区、current-state.md 旧描述与矛盾文本。质量门：744/744 TS 测试（23 文件），lint 0 errors，build pass，253 Python 通过，Codex 独立验收通过。） |
...
- `src/features/sync/actions.ts` — `createSyncActions` 新增 `runAutoPreReview()` 方法
- `src/features/sync/p5-sy10c-auto-pre-review.test.ts` — 30 项测试
...
2. Session health check → unhealthy 则全局 BLOCK，不执行 Dry Run
3. `getCachedOverseasWarehouses()` → 获取全部活跃海外仓
4. **逐仓预取 `getWarehouseHistory(warehouseId)`**（在 Dry Run 之前，缓存到 `Map<warehouseId, WarehouseHistory>`）
5. `triggerBatchDryRun(warehouses)` — 复用 P5-SY9F 管线
6. 逐仓用**预取历史**调 `evaluateRules(input)` —— 本次 Dry Run 不污染规则评估历史
7. 历史获取失败的仓库 → BLOCK（`history_unavailable` 规则），不 fallback 到冷启动
8. 返回 `AutoPreReviewResult`
...
interface AutoPreReviewItem {
  ...
  /** 仓库历史同步上下文（Dry Run 前预取，不含当前 run） */
...
- 正常流程：逐仓预取历史 → Dry Run → 逐仓规则评估（使用预取历史）→ 完整 AutoPreReviewResult
- 冷启动仓库 `history.hasBaseline` 必须为 `false`（预取历史不含当前 Dry Run 记录）
- 已有历史仓库的 `history.stats` 不包含当前 run 的数据
- 单仓 Dry Run 失败不影响其他仓的规则评估
- 单仓 `getWarehouseHistory` 失败 → 该仓 BLOCK（`history_unavailable` 规则），包含中文原因；其他仓继续
- 不触发任何 Real Write
- Admin 权限校验有效
...
> 注意：Vercel Hobby 计划仅支持每日一次 cron。生产环境可调整为更高频。当前配置 UTC 01:00 = 北京时间 09:00。cron 时间使用 UTC，需在注释中标注对应的北京时间。
