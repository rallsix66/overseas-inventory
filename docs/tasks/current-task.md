...
8. Supabase 生产库验证通过：`shipment.purchase_order_no` 字段存在，`create_shipment_transactional` / `change_shipment_status_transactional` 均为 Admin-only ✓
9. `npm run test` 1603/1603（49 文件，concurrency/best live 预存失败），`npm run lint` 0 errors / 26 warnings（all pre-existing），`npm run build` 通过，`git diff --check` 通过 ✓
