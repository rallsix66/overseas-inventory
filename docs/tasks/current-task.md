...
`IN_PROGRESS` — P5-SY10A DONE（规则引擎核心），P5-SY10B DONE（历史上下文提供器：consecutiveFailures 仅统计 mode='dry_run' 且 status='failed'，real_write/completed/in_progress 均作为屏障），P5-SY10C DONE（自动预审编排 Server Action，Codex 独立复验通过），P5-SY10D IN_PROGRESS（预审页面 UI）。P5-SY10E~F 待启动。P5-SY9 全部子任务（A~K）DONE。
...
| **P5-SY10D** | 预审页面 UI：扩展批量 Dry Run 对话框展示规则决策 | 在 BatchReviewCard 上新增规则决策徽标（PASS 绿 / WARN 黄 / BLOCK 红）。新增规则详情展开区。WARN 仓库仍可选（带警告提示），BLOCK 仓库不可选。扩展批量 Dry Run 对话框统计栏。 | P5-SY10C | IN_PROGRESS |
...
- `src/features/sync/p5-sy10c-auto-pre-review.test.ts` — 26 项测试
