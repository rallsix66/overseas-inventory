...
`IN_PROGRESS` — P5-SY10A DONE（规则引擎核心），P5-SY10B DONE（历史上下文提供器：consecutiveFailures 仅统计 mode='dry_run' 且 status='failed'，real_write/completed/in_progress 均作为屏障），P5-SY10C DONE（自动预审编排 Server Action，Codex 独立复验通过），P5-SY10D AWAITING_REVIEW（预审页面 UI：自动预审入口 + RuleBadge + 可展开规则详情 + 复选框返工：独立 `autoReviewSelectedItems` Set + PASS/WARN 可选/BLOCK 不可选，29 项新测试，681/681 TS）。P5-SY10E~F 待启动。P5-SY9 全部子任务（A~K）DONE。
...
| **P5-SY10D** | 预审页面 UI：自动预审入口 + 规则决策徽标 + 可展开规则详情 | Sync 页面新增「自动预审」按钮（调用 `runAutoPreReview()`）。BatchReviewCard 扩展 `ruleVerdict` prop：RuleBadge（PASS 绿/WARN 黄/BLOCK 红）+ 可展开规则详情（evaluations[].message）+ 复选框返工：独立 `autoReviewSelectedItems` Set + PASS/WARN + status=ready 可选 / BLOCK/failed/blocked 不可选。统计栏 PASS/WARN/BLOCK 计数。保留「批量 Dry Run」按钮。Operator 只读。清理 unused imports。29 项新测试（含 8 项复选框行为源码检查）。 | P5-SY10C | AWAITING_REVIEW |
