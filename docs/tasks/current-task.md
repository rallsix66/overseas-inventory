...
**OPT-6-PROGRESSIVE-QUALITY-GOVERNANCE 鈥?BATCH 2 REMOTE APPLY/POSTCHECK PASS / BATCH 3 IMPLEMENTATION REVIEW PASS / STAGING SELECT-ONLY PREFLIGHT CORRECTION REVIEW PENDING**
...
`implement 鈫?complete evidence/quality verification 鈫?designated independent review 鈫?explicit PASS 鈫?next stage`.
...
- PR #9 was merged as `d9acf51e0cfbfd2e21f243f41273de7278f4e80a`;
  ...
  applied 00050 to Staging and Production. Both are now exact `00001`鈥揱00050`,
...
1. 鉁?Create this isolated branch and record the OPT-5 handoff.
2. 鉁?Re-run lint and collect a machine-readable warning inventory; fix unused symbols in small test-backed batches until warning count is zero.
3. 鉁?Batch 1: capture the reviewed policy targets, rewrite only six `auth.uid()` init expressions to equivalent scalar subqueries, and prove anonymous, disabled, Admin, Operator, and cross-warehouse behavior unchanged. See [Batch 1 report](../reports/2026-07-20-opt6-quality-governance-batch-1.md).
4. Inventory multiple-permissive policies by table/command/role. The local
   `00001`鈥揱00050` replay catalog has 42 policies and 23 concrete overlap
...
- Staging read-only preflight subsequently passed: exact `00001`鈥揱00050`, no
...
- A separate SELECT-only postcheck returned one row with all nine checks
  `true`: exact 00001鈥?0051 history, unique version/name sets, no timestamp
...
- Its `expected_history` CTE pins all reviewed `00001`鈥揱00050` version/name and
...
- The packet was executed read-only on 2026-07-22 and all history, full-payload,
  ...
  exact 00001鈥?0051 history, the canonical payload, four role policies, and
  zero active sync runs. Batch 3 remains a separate implementation/review gate.

## Current prohibitions

- No changes to 00001鈥?0049; all database changes must be 00050+ forward-only and replayable.
