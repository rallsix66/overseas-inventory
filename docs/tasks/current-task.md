...
`IN_PROGRESS` — P5-SY10A DONE（规则引擎核心），P5-SY10B 返工完成待 Codex 复验（历史上下文提供器：consecutiveFailures 仅 dry_run failed，real_write 屏障）。P5-SY10C~F 待启动。P5-SY9 全部子任务（A~K）DONE。
...
| **P5-SY10B** | 历史上下文提供器：基线追踪 + 连续失败检测 | 实现 `getWarehouseHistory()` 从 sync_run 推导 hasBaseline / consecutiveFailures（仅 dry_run failed，real_write 屏障）/ lastSuccess / stats。走 Repository 接口。 | P5-SY10A | **返工完成待 Codex 复验**（2026-06-24。`SyncRepository.getWarehouseHistory()` + MockRepository + SupabaseSyncRepository 实现。`buildResultSummary()` 持久化 scraperMeta。35 项测试（新增 4 项 real_write 屏障测试）。621/621 TS，lint 0 errors，build pass。） |
