-- OPT-4 Staging history-only version realignment.
-- Target project: hyarhvsjhkjpallbyifn (Staging) only.
--
-- This is a maintenance-window script, not a Migration. Keep it outside
-- supabase/migrations/. It must not be run against Production and must not be
-- run before the user explicitly approves the Staging history-only window.
--
