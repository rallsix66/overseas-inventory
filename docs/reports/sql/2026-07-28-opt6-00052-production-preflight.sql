-- OPT-6 Batch 3 / 00052 Production exact preflight (read-only).
...
-- transaction, execute a Migration, alter policy, or register 00052. Run it
-- only as the Production SELECT-only preflight before a separately reviewed Production maintenance
...
-- Reviewed Production baseline; 00041-00047 retain the Production payload variant. The statement digest is over the complete
  ...
  SELECT
    ...
    array_agg(version ORDER BY version) IS NOT DISTINCT FROM
      ARRAY(
        ...
        FROM generate_series(1, 51) AS series(i)
    ) AS exact_version_set,
    NOT EXISTS (
      SELECT 1 FROM history WHERE version = '00052'
    ) AS no_00052
  FROM history
),
-- Reviewed Production expected digests: version/name 2d6174dce487614c3280456fff9169d0; full payload 0b7cba5a88fff139fb0ec65e4deaa142.
history_compare AS (
  SELECT
    ...
    md5((SELECT string_agg(version || '|' || name, E'\n' ORDER BY version) FROM expected_history)) = '2d6174dce487614c3280456fff9169d0' AS expected_version_name_digest_is_reviewed,
    md5(string_agg(
    ...
    md5((SELECT string_agg(version || '|' || name || '|' || statement_count || '|' || statement_chars || '|' || statement_digest, E'\n' ORDER BY version) FROM expected_history)) = '0b7cba5a88fff139fb0ec65e4deaa142' AS expected_history_payload_digest_is_reviewed,
    (
...
expected_product_policy(policy_name, permissive, roles, command, using_expression, with_check_expression) AS (
  VALUES
