-- OPT-6 Batch 3 / 00052 Production apply packet (PREPARED; do not run without a separately reviewed window).
-- Target: Production project hzlhqyditalumhnxbaim only.
--
-- This packet locks migration history and sync runs before any policy DDL,
-- revalidates the approved 00001-00051 full statements[] baseline in-transaction,
-- and refuses any history, policy, or active-sync drift. It is preparation only.
...
VALUES('00001', '00001_initial_schema', 1, 15219, 'b9ffd51f5f16c72c95a86a55ab053419'),
...
BEGIN
  IF (SELECT count(*) FROM public.sync_run WHERE status = 'in_progress') <> 0 THEN
    RAISE EXCEPTION '00052 apply preflight failed: active sync run';
  END IF;

  IF EXISTS (SELECT 1 FROM supabase_migrations.schema_migrations WHERE version = '00052') THEN
    RAISE EXCEPTION '00052 apply preflight failed: version already registered';
  END IF;
  ...
  SELECT
    (SELECT count(*) FROM actual_history) = 51
    AND NOT EXISTS (
      SELECT 1
  ...
  IF NOT exact_history THEN
    RAISE EXCEPTION '00052 apply preflight failed: exact 00001-00051 history drift';
  END IF;
END;
$$;

CREATE TEMP TABLE opt6_product_apply_policy_expected (
...
INSERT INTO opt6_product_apply_policy_expected VALUES
...
BEGIN
  ...
  SELECT count(*), count(actual.policy_name), count(*) FILTER (
  ...
  INTO policy_count, matched_count, mismatch_count
  FROM opt6_product_apply_policy_expected expected
  FULL JOIN actual USING (policy_name);

  IF policy_count <> 2 OR matched_count <> 2 OR mismatch_count <> 0 THEN
    RAISE EXCEPTION '00052 apply preflight failed: exact product policy baseline drift';
  END IF;
END;
$$;

-- Migration 00052: remove the reviewed product-table permissive policy overlap.-- Migration 00052: remove the reviewed product-table permissive policy overlap.
--
...
WITH body AS (
  SELECT replace($migration$-- Migration 00052: remove the reviewed product-table permissive policy overlap.
--
-- The existing admin FOR ALL policy and operator SELECT policy are both
-- permissive for SELECT. This migration preserves their union exactly while
-- leaving one policy per command: one shared read policy and three admin-only
...
SELECT '00052', '00052_optimize_product_rls_policy_overlap', ARRAY[value] FROM body
WHERE length(value) = 5786 AND md5(value) = '580fd279b2f8d07f6c5a550acc82812a';
TRUNCATE opt6_product_apply_policy_expected;
INSERT INTO opt6_product_apply_policy_expected VALUES
  ('product_select_admin_or_operator', true, '{0}', 'r', '((get_user_role()=''admin''::text)OR(get_user_role()=''operator''::text))', ''),
  ('product_insert_admin', true, '{0}', 'a', '', '(get_user_role()=''admin''::text)'),
  ('product_update_admin', true, '{0}', 'w', '(get_user_role()=''admin''::text)', '(get_user_role()=''admin''::text)'),
  ('product_delete_admin', true, '{0}', 'd', '(get_user_role()=''admin''::text)', '');
...
BEGIN
  ...
  IF (SELECT count(*) FROM supabase_migrations.schema_migrations) <> 52
     OR NOT EXISTS (
       ...
       WHERE version = '00052' AND name = '00052_optimize_product_rls_policy_overlap'
         AND cardinality(statements) = 1 AND length(statements[1]) = 5786
         AND md5(statements[1]) = '580fd279b2f8d07f6c5a550acc82812a'
     )
     OR NOT exact_history
     OR policy_count <> 4 OR matched_count <> 4 OR mismatch_count <> 0
     OR (SELECT count(*) FROM public.sync_run WHERE status = 'in_progress') <> 0 THEN
    RAISE EXCEPTION '00052 apply postcheck failed; transaction will roll back';
  END IF;
END;
$$;
